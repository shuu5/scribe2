...
- `append(dir, &Event) -> Result<Warnings, StoreError>`: lock file を `create_new` で取り（再試行の上限は rules 行 `fleet.lock_retry_ms`）、`O_APPEND` で 1 行書いて flush し、lock を外す。**rules 行 `fleet.lock_stale_ms` より古い lock は stale として除去し** `Warning::StaleLockRemoved` を返り値に載せる（黙って消さない）。lock file には**所有者の pid を 10 進 1 行**で書く（`create_new` で開いた handle にそのまま書く・第 2 の writer を作らない）。既存の lock に当たった周は中身を読み、**所有者が生きていなければ外して取り直し**、その旨を warning の 1 種として返り値に載せる（黙って消さない）。読めない中身と生きている pid は従来どおり `fleet.lock_stale_ms` の線に従う（FailClosed の極性は変えない）。判定は純関数 1 本（所有者は 3 値: 死んでいる / 生きている / 読めない）で、pid の生存判定（起動時刻）の実装は受付の札（[ADR-0021 §2.3](../../design-intent/decisions/ADR-0021-gate-cost-is-measured-and-confined.html#s2-3-slots)）と共有する 1 本にする（C6.3）。中身が pid だけである理由: 札の ts を持たないので pid の再利用は弁別できないが、再利用された pid は「生きている」と読んで**待つ側**へ倒れる（安全な向き）＝札と同じ 2 値を持たせると第 2 の受付が生える。新しい閾値は作らない（rules 行も裁定 id も増えない）。
