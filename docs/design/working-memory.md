...
- marker 4 種（`[RUN]` / `[SEAT]` / `[WIN]` は各 3 形: 列挙 / `-COUNT` / `-NONE`。`[MAIN]` は値 1 行ゆえ列挙だけで、`[WIN]` は着地を読めない周の `-UNKNOWN` 形を持つ＝variant は 11）: `[MAIN] sha=<短 sha> origin=<same|ahead|behind|diverged|unknown> porcelain=<n>`（anchor の git・既存の git の子 process・`diverged` = 双方に相手に無い commit が在る周〔`rev-list --left-right --count` の両側 > 0〕で、読めない周の `unknown` とは別の値・.326.1 run 1 審査 FAIL 2026-09-16「節に無い 5 値目」の解として本文に足した）/ `[RUN] id=<run> stage=<Stage> account=<label> base=<sha> updated=<ts>`（event log の `RunStage` の最終値・終端〔Landed / Failed / Retired〕を除く）/ `[SEAT] target=… role=… state=<idle|busy> account=… model=…`（`SeatRegistered` の最終 row + state.jsonl の最終打刻）/ `[WIN] id=<bead> landed=<ts> sha=<短 sha>`（直近の自席の `.consumed.md` の `externalized_at` より後の Landed）。
