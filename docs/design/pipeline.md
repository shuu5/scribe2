...
- **不可逆の口（CON5・N1）**: force 系 git・削除の subcommand は無い（後始末は可逆 move・§5.4）。**「出す」= repo の外へ出す操作**（他 repo への push・外部サービスへの送信・release / tag の公開）で、core はその口を持たない。自 repo への branch push と PR 作成（`--pr-cmd`・§5.4）は A4.3 で可逆ゆえ「出す」に当たらず、承認 event を前提としない（[ADR-0008](../../design-intent/decisions/ADR-0008-own-repo-pr-is-not-publish.html)）。**課金**: runner / lens は定額の subscription 内で走り、追加課金の口を持たない（「使う」= 追加課金だけ・要件カタログ R-F4）。
...
- 3 クラスの判定は MVP では**契約の自己申告**（`classes`）だけである（**seam の使用からの導出は [ADR-0008](../../design-intent/decisions/ADR-0008-own-repo-pr-is-not-publish.html) で廃止**した——`--pr-cmd` は自 repo への PR の口ゆえ承認 event を前提としない・§5.4）。操作の中身から 3 クラスを判定する enforcer は次の版（A4 の機構欄）。
