...
- 決定: [ADR-0020](../../design-intent/decisions/ADR-0020-account-autonomy-limit-stop-resume-and-selection.html)（本 doc の決定の正本・§2.1〜§2.6）/ [ADR-0012](../../design-intent/decisions/ADR-0012-rate-limit-detection-reads-dedicated-record.html)（上限 record）/ [ADR-0017](../../design-intent/decisions/ADR-0017-account-allowance-is-measured-by-host-http-client.html)（実測行・label・credential の場所）/ [ADR-0022](../../design-intent/decisions/ADR-0022-seat-roles-are-typed-and-enforced-by-hooks.html) §2.1（登録 row）
...
上限に当たった便は Failed（`detail=rate-limit`）で終端し、worktree と回答が捨てられて新しい便でしか続けられない。口座は席の起動時に人が選び、便は席の口座を継承するので席の口座が食い潰される。席はモデル別 7 日窓だけに当たって止まった（記録時点の事故型）。器は口座残量（FR33・実測行）を持つが選定に使っていない。

## 2. 上限停止の段（ADR-0020 §2.1・FR35）

- **`Stage::RateLimited`**（variant 1 つ・宣言順は `Questioned` の直後・`as_str` は `RateLimited`）。runner が上限の rc（`RC_RATE_LIMIT` = 75・ADR-0012 §2.1）で終わった周は `RunStage stage=RateLimited detail=account:<label>,window:<WindowKind>` を記帳する（label は不透明・窓は `WindowKind` の値）。上限の判定入力は上限 record の status のみ（ADR-0012・本 doc は判定を足さない）。
...
- **入力**（2 種 + 用途）: manifest の `[[account]]` 行の label 列（宣言値）・口座 × 窓ごとの**最新の実測行**（`AllowanceMeasured` / `AllowanceUnmeasured`・実測値・reset 時刻付き）・用途（`Purpose::{Run, Session}`・closed enum）・使う model（display name・モデル別 7 日窓の選択に使う）・除外する label の集合（席の登録 row が持つ口座・[seat-roles.md](./seat-roles.md) §2）・rules 行の値（session 用の閾値）。**席の判断・pane の字面・前の版の道具の出力は入力にしない**。
- **出力**: `Selection::{Chosen(label), None(NoCandidate)}`。`NoCandidate` は typed な理由（全口座が当たっている / 測れない / 除外で空）と**最も早い reset 時刻**（§4 の待ちに渡す）を運ぶ。
- **逼迫度** = その口座の窓のうち最大の使用率（5 時間・7 日・用途が使う model のモデル別 7 日窓。他の model の窓は数えない）。**当たっている** = 最新の実測行の使用率が 100（窓の全量・規則値ではない）に達している。
...
- **R-C9-1**: kind は既存の `AccountSelection` のまま、値を散文（Str・enabled = false）から **Int**（session 用の閾値・百分率）に改めて enabled = true。値と裁定 id は契約 (b) が manifest に書く（**user 裁定**・§9）。便用の規則は閾値を持たない。
...
- **候補なしの周**: `Completion::AccountFree { reset_at }`（variant 1 つ・運ぶ値は pid でなく最も早い reset 時刻＝現物の完了 enum の `pid()` は 0 を返す形〔`SlotFree` と同じ〕）を足して唯一の wait 実装を通す。deadline は reset 時刻から計算した値（rules 行ではない・縮退を持たない）。Timeout（C11.3 の Result）を受けた周は計測して選び直し、候補なしなら次の reset 時刻で待ち直す。起こし直しの回数に上限を持たない（窓を跨いで続く）。待ちの間も便は live。終端手段は `pipe stop --run` だけ（器は自動では終端しない）。
...
- 段: 偽 runner が上限 record を最終行に書いて rc 75 で終わる fixture で `RateLimited` の段・detail に label と窓・Failed でない・worktree と commit が残る・`pipe show` が段名を出す・intake の排他が本段の便を live に数える・`pipe stop --run` で止められる。
- 選定（in-file・純関数）: 便用は逼迫度最大の当たっていない口座・席の口座は除外・session 用は逼迫度最小かつ閾値未満・当たっている口座（100）は選ばない・Unmeasured / 古い行の口座は候補外・同点は辞書順・全部当たっているときは `NoCandidate` に最も早い reset・model 別窓は用途の model だけ数える。
- 再開: 偽 runner を 2 回起こす fixture（1 回目 rc 75・2 回目で commit）で別 label の `--account-dir` が渡り（stub が argv を写す）・stdin に「途中再開」節と順序（契約 → 回答 → 途中再開）・`Spawned detail=account:…,resume:rate-limit`・Landed まで通る（AC12）／全口座が当たっている fixture で `AccountFree` の待ち → 実測行を新しくすると待ちが解ける（deadline を fixture で短く）／`pipe stop --run` で待ちの便を終端できる。
...
- **(a) 上限停止の段**（S）: `Stage::RateLimited`・runner の rc 75 → 段の記帳（label / 窓）・`pipe show`・排他の母集団・stop。write-set = pipe/mod.rs・pipe/spawn.rs・pipe/cli.rs・tests/e2e/pipe.rs・snapshot。依存: なし（ADR-0012 の rc 75 は現物）。
...
- **(c) 別口座での途中再開**（M）: `Completion::AccountFree`・`pipe run` / `resume` の RateLimited の経路・「途中再開」節・`--account-dir` の配線・記帳。write-set = pipe/follow.rs・pipe/cli.rs・pipe/spawn.rs・fleet/mod.rs・headless/runner.txt・tests/e2e/pipe.rs。依存: (a)(b)・s2-07l.147（land の順序制御・pipe/ と fleet/mod.rs を触るので直列）。
