...
- 形（行 f・1 つずつ歯が測る・done と 1:1）:
  ...
  5. **人向けの案内の表も同じ便で動かす**: `init` の使い方の行（usage）に `[--ledger-prefix <P>]` が増えるので、`crates/scribe2/src/help.rs` の表の `init` の FORM（usage の 1 行目と一致・cli-help.md 行 a）と FLAGS に同じ 1 語を足す。表の他の口は 1 字も変えない（既存の歯 `cli_help_pages_match_the_live_form_and_every_subcommand` が FORM と live の usage の一致を測る＝足さなければ赤）。
