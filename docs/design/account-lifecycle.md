...
verify = ["cargo nextest run -p scribe2-boundary --test e2e --no-tests=fail seat_launch_scope_", "cargo nextest run -p scribe2-boundary --test e2e --no-tests=fail seat_tick_wake_", "cargo nextest run -p scribe2-boundary --test e2e --no-tests=fail rules_seat_box_", "cargo nextest run -p scribe2-boundary --test e2e --no-tests=fail rules_external_form", "cargo nextest run -p scribe2-boundary --test e2e --no-tests=fail seat_usage_external_form", "cargo nextest run -p scribe2-boundary --test e2e --no-tests=fail rules_embedded_manifest_is_valid_and_covers_all_kinds", "cargo nextest run -p scribe2-boundary --test e2e --no-tests=fail rules_embedded_manifest_declares_host_guard_kinds_at_the_tail_of_all", "cargo nextest run -p scribe2 --lib --no-tests=fail seat_launch_derive_line_", "cargo nextest run -p scribe2 --lib --no-tests=fail confine_scope_args_"]
size = "M"
growth = ["crates/scribe2/src/rules/mod.rs:8", "crates/scribe2/src/seat/cycle/launch.rs:50", "crates/scribe2/src/pipe/confine.rs:40", "crates/scribe2/src/seat/cli.rs:24", "crates/scribe2/src/help.rs:4", "crates/scribe2/src/seat/tick.rs:6", "crates/scribe2/src/pipe/dispatch/group.rs:6", "crates/scribe2/src/pipe/follow.rs:6"]
done = "(1) 埋め込み manifest に seat.memory_max_mb（kind SeatMemoryMaxMb・値 32768・裁定 id user 2026-09-26T08:28Z）が SeatMoveGraceS の直後の宣言順で在り（rules_seat_box_ の歯が id / kind / 値 / 裁定 id / 隣接を pin）rows=71 kinds=69 が rules validate の外形と snapshot に載る (2) confine.rs の scope_args が CPUWeight の有無の引数で 2 形になり、席の頭の語列は pub な 1 関数（unit 名と MiB）で systemd-run --user --scope --quiet --collect --unit=<name> -p MemoryMax=<mb>M -p OOMPolicy=continue -- の順・unit 名は unit_name(潰した target, seat, 0) (3) derive_launch が頭の語列を Option で受け env 3 語の直後に置き、None は 1 字も変わらず、Launch の欄 1 つ（行の読み）を 4 つの構築点が自分の manifest から埋め（seat launch は --rules F の写し・無ければ埋め込み・使い方の 1 行と help の form に [--rules F] が増える）、launch は値 0 / 読めない周と PATH に実行可能な systemd-run が無い周は頭を付けず（probe は撃たない）、登録 row の launch は素の導出行のまま、cd の前置と会話の引き継ぎの語は不変 (4) render_launched の launched の 2 形の末尾に scope=<unit 名|off|no-rules|no-systemd-run> が付き Refused / None / Failed には付かず、tick の判定行と群の段の記録と event は不変 (5) §4 の口・口座の選定・登録 row・trust・便の封じ込めの Wrap / probe / release・pane_is_shell・§4.5 は不変で、seat_launch_scope_（道具箱を積んだ起動行の形と unit 名の形と scope=<unit>・道具箱なしの素の行と scope=no-systemd-run・--rules で行 0 の素の行と scope=off・行を欠く写しの素の行と scope=no-rules）・seat_tick_wake_ の --rules で 0 と行なしの素の行・既存 seat_launch_ 31 本と seat_entry_ 5 本と seat_defaults_ 4 本の期待の行の頭（seat/launch.rs の起動の歯は全部 PATH を固定・seat.rs / dispatch.rs / fleet.rs の読み手は頭に依らず不変で触らない）・lib の derive_line と scope_args の 2 形・rules_seat_box_ の行の pin と順と snapshot・usage の snapshot を測る"
<!-- contracts:end -->
