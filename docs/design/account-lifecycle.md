...
- 形（行 r・1 つずつ歯が測る・done と 1:1）:
  ...
  5. **doctor の群の行は `next=<label|none|unreadable|no-rule>`** を `current=` の後ろに足す（同じ予約の関数・測らない〔鮮度の外の候補は門を通らない＝none になりうる〕・記録を読めない群は unreadable・役割の model の行が無い周は no-rule・群 0 の host は 1 語も変わらない）。
