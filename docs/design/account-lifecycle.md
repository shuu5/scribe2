...
- 何が起きているか（別 repo の planner 席からの relay 2026-09-17・出所と記録の字面は台帳 `s2-07l.438`）: 席を credential dir だけ替えて手で起こし直すと、実 session は口座 A で動くのに登録 row（`SeatRegistered` の account）は口座 B のまま残る。現物（main 3f1eec8）: tick の口座の軸（管理 tick の口座の軸〔削除済み〕 の `seated` → `reading`）は **row の口座**の逼迫度だけを読むので、B が閾値以上の周に context 14% の席へ `origin=account` の退避の合図を注入した（誤発火）。SessionStart の指示文（`seat/brief/`）は row の値だけを出し、食い違いを名指す面が 1 つも無い。row を実測へ寄せる書き手も無い（SessionStart は row を書かない）。
