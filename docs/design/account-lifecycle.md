...
- **外の wrapper との関係**: 前の版の launcher が持つ cgroup の防壁は席には持ち込まない（席は shell の子で器の子ではない・立て直しと同じ・NFR6 の「器の子 process」に当たらない）。**§30（ADR-0072）で覆す**: 席の起動行の頭に器の封じ込めの箱を付ける（前の版の launcher の防壁を戻すのではなく、便と同じ scope の語列を rules 行 1 本の上限で付ける）。plugin の自動検出は `[[plugin]]` 行の宣言で置き換える（走査しない・C3）。
...
- 席の起動に cgroup の防壁を持ち込む。却下: 席は shell の子で器の子ではない（立て直しと同型）。防壁が要るのは便の runner で、それは [gate-cost.md](./gate-cost.md) の封じ込めが持つ。**§30 で却下を取り消し**（2026-09-25 の login 不能・裁定 2026-09-26T08:28Z）。
...
- 形（行 r・1 つずつ歯が測る・done と 1:1）:
  1. **残量の鍵は pure な 1 関数**: 口座の鮮度の内側の実測の行から (a) 残量 = min(100 − 7 日窓の使用率, 役割の model ごとの 100 − モデル別 7 日窓の使用率) の大きい順 → (b) 7 日窓の `resets_at` の早い順（無い口座は後）→ (c) 宣言の候補の index の順、の辞書順。役割の model の集合 = 群の置き場の席の登録 row の役割（重複は畳む）ごとの rules 行 `seat.model.<役割>` の表示名。集合のどれか 1 つでもモデル別窓の行を持たない口座は最後（(a) の前に立つ bool）。集合が空（席の row が無い群）は 7 日窓だけで並べる。役割の行が無い周（群の席の役割に `seat.model.<役割>` の rules 行が無い）は typed に止まる（`Caps` の行が無い周と同じ極性・既定を出さない・集合を空に読み替えて 7 日窓だけで並べない）: 1 周の群の段は移らず記録 0・承認 event 0・断りの event 0 で typed に止まり（lock の内側の判定が `Unreadable` を返す・便の列の rc は変わらない）、tick の判定行は `judged=error`、doctor の `next=` は `no-rule`（label / none / unreadable に潰さない・C10）。5 時間窓は門にだけ使う。
  2. **群の予約は 1 関数**（鍵の関数を呼ぶ）: 群を宣言順に見て、群 g の予約 = 門（今のまま: 今の口座でなく・どの群の今の口座でもなく・退役中でなく・鮮度の内側の実測を持ち 3 窓とも閾値未満・鮮度の外の候補は `measure` で 1 回測る）を通る候補を鍵で並べたものから、先の群の予約でない先頭（無ければ無し）。移る群の移り先 = 自分の予約。`target_of` はこの関数に置き換わる（宣言順で最初に門を通る label は返さない）。同じ周に先の群が移った後も、先の群の予約は周の頭の今の口座から導く（移った群は自分の予約を使った＝後の群はその次へ・判じる群の門は周の中で更新した今の口座を外す＝2 群が同じ周に同じ label へ移らない）。
  ...
  5. **doctor の群の行は `next=<label|none|unreadable|no-rule>`** を `current=` の後ろに足す（同じ予約の関数・測らない〔鮮度の外の候補は門を通らない＝none になりうる〕・記録を読めない群は unreadable・役割の model の行が無い周は no-rule〔閾値と鮮度の rules 行が無い周も同じ no-rule＝rules 行の無さを unreadable に潰さない〕・群 0 の host は 1 語も変わらない）。
...
- 形（行 s・1 つずつ歯が測る・done と 1:1）:
  ...
  4. **fixture の群の名を Tier と数字に改める**（e2e 8 file）: 名だけを改め assert の他の語は変えない。`init --group` の周の fixture も同じ。母集団（`[[account-group]]` の字面を持つ helper / const から名の到達で数えた歯・2026-09-26 の census・main 66e0089〔行 o の着地後〕）: rules.rs 7 本（`host_group_defects_` / `host_group_seed_` / `host_group_table_` / `rules_review_same_`）・main.rs 7 本（`host_init_` / `init_repo_` / `init_seat_`）・pipe/dispatch.rs 40 本（`pipe_dispatch_group_`）・fleet.rs 8 本（`host_group_`）・seat/launch.rs 18 本（`seat_launch_`）・seat.rs 33 本（`seat_doctor_run_` / `seat_pane_shell_` / `seat_tick_` / `seat_heartbeat_`〔行 o が足した群の判定と退避の歯〕）・seat/account.rs 5 本（`host_group_`）・hook.rs 13 本（`hook_group_`）＝**131 本**。行 s の検証行はこの接頭辞を全部持ち、名の改め漏れ（旧い名の群を持つ面が読みで断られて歯が赤くなる）を検証行が測る。**改名だけの歯の file（rules.rs 以外の 7 file）は base でも緑**（名の改めは base の規則を 1 つも破らない）なので、gate の flip-check が `green-on-base file=…` で落とす（run 023931Z の再現・fleet.rs の overlay が全部 PASS）＝file ごとに札 `// flip-check: retroactive <行 s の bead>` を**1 行**置く（e2e の file は全体が歯の区間・効く条件は test 区間内 / 行頭 / bead id 必須 / base に無い札の 4 つ・[contract-source.md](./contract-source.md) の `.277` の逃がしと同じ形）。札は緑を免じる印でなく「後から動かした歯」の申告なので、done の変異 A/B（Tier でない名を通す変異・昇順を辞書順で比べる変異が `host_group_tier_` で落ちる本数と母集団）を notes に残す。rules.rs は新しい歯 `host_group_tier_` が base で赤なので札は要らない。
...
- 歯: `crates/scribe2-boundary/tests/e2e/pipe/dispatch.rs`（`pipe_dispatch_group_reserve_` 接頭辞・§20 の `pipe_dispatch_group_move_` と同じ偽 usage の fixture〔候補の窓の値を口座ごとに与える〕・行 s の後なので群の名は Tier1 / Tier2）: (a) 宣言順で先の候補の残量が小さく後の候補の残量が大きい周は後の候補へ移る（base では先の候補 ＝ RED）(b) 7 日窓の残量が同じで役割の model の窓の残量が違う 2 候補は model の残量の大きい方へ（base では宣言順 ＝ RED）(c) 残量の同点は 7 日窓の reset の早い方へ、reset も同じなら宣言順（2 本）(d) 役割の model のモデル別窓の行を持たない候補は残量に依らず最後へ（base では宣言順 ＝ RED）(e) 5 時間窓が閾値以上の候補は門で落ち、閾値未満の周は 5 時間窓の値が並びを変えない（2 本）(f) 2 群が同じ周に逼迫し候補を共有する周は Tier1 が鍵の先頭へ、Tier2 は次へ（記録 2 の label が異なる・鍵の先頭 ≠ 宣言の先頭で組み、base の宣言順の更新と同じ結果になる fixture を避ける）(g) Tier2 だけが逼迫し Tier1 は逼迫でない周も、Tier2 は Tier1 の予約（Tier1 の鍵の先頭）を飛ばして 2 番目へ移る（base では Tier1 の予約を取る ＝ RED・持ち主の裁定の核）(h) 予約の立つ口座（Tier1 の鍵の先頭・記録は動かない）を便用の選定が使い、群用 dir に予約の file は 0（base では便も使う＝GREEN なので、この歯は (g) の fixture の中で同じ周の便用の選定を測る 1 assert にする）(i) Tier2 だけが逼迫する周に Tier1 の候補（鮮度の外）が測られる（偽 usage の呼び出しに Tier1 の候補の label が在る・base では無い ＝ RED）(j) 群の席の役割の `seat.model.<役割>` 行を欠く `--rules` で逼迫の周を撃つと移らず記録 0・承認 event 0・断りの event 0 で便の列の rc は不変（base では宣言順の先頭へ移る ＝ RED・集合を空に読み替える変異はここで落ちる）。`crates/scribe2-boundary/tests/e2e/seat.rs`（`seat_tick_judge_reserve_` 接頭辞・§9 の `seat_tick_judge_` の fixture）: tick の判定も Tier1 の予約を飛ばす（base では RED）／ 役割の行を欠く `--rules` の tick は `judged=error` で移らず 0 key（base では移る ＝ RED）。`crates/scribe2-boundary/tests/e2e/seat/account.rs`（`host_group_next_` 接頭辞）: doctor の群の行の `next=` が鍵の先頭の label ／ 門を通る候補が無ければ none ／ 記録を読めない群は unreadable ／ 役割の行を欠く `--rules` は no-rule（base では `next=` 無し ＝ RED）／ 既存の `host_group_doctor_line_says_none_without_seat_rows` は行の末尾に `next=` が増えるので書き換えて GREEN（base では `next=` 無し ＝ RED）。lib（`crates/scribe2/src/hook/group.rs`・`group_key_` 接頭辞）: 鍵の関数の並びを行の値だけで測る（役割の集合が空・1 つ・2 つ・model の行を欠く口座）。行 s は `crates/scribe2-boundary/tests/e2e/rules.rs`（`host_group_tier_` 接頭辞・§28 の `host_group_seed_` の fixture）: (j) 名が alpha の群は欠陥（行番号は群の行・語「Tier と数字の形でない」・base では通る ＝ RED）(k) Tier2, Tier1 の順は 2 番目の群の行の欠陥（語「前の群より大きくない」）(l) Tier1, Tier1 は名の重複の 1 件だけ（昇順の欠陥を重ねない・既存の歯 `host_group_defects_are_refused_with_line_numbers_and_the_face_prefix` の名の重複の本文が名の改めだけで 1 件のまま GREEN） (m) Tier01 は形の欠陥 (n) Tier1, Tier2, Tier10 は通る（数値で比べる・辞書順で Tier10 < Tier2 と読む変異を捕まえる）。既存の群の歯（上の 131 本）は名の改めだけで GREEN＝検証行の接頭辞 13 本がその全部を撃つ。行 o のように**後から群の fixture に到達する歯が増えた周は census と検証行を同じ PR で足す**（run 031044Z の審査の再現）。

## 30. 席の起動を封じ込めの箱で包む — rules 行 seat.memory_max_mb（32768）・起動行の頭の systemd-run --scope（unit 名は便と同じ 1 関数・CPUWeight 無し・OOMPolicy=continue）・道具が PATH に無い host と行が 0 / 読めない周は素の行（契約表の行 t・ADR-0072・FR59 / NFR6・`s2-07l.627`）

やさしく言うと: 便（runner / lens / 検証行）は memory の上限つきの箱（systemd の scope）の中で走るので、溢れても便 1 本が落ちるだけで済む。席（人と話す claude とその subagent の Bash）は箱が無く、user の slice に直に入る。2026-09-25 に席の subagent が撃った sed 1 本が RSS 81 GB まで膨らみ、slice 全体が詰まって ssh の login まで止まった。そこで席の起動行の頭にも同じ箱（上限は rules 行 1 本・32 GiB）を付ける。箱の中で溢れたときに殺されるのは箱の中の 1 process だけで、claude が死んだ周は窓が shell に戻り管理 tick が起こし直す。systemd-run が無い host や行が 0 の周は今までどおりの素の行。

- 出所: 台帳 `s2-07l.627`（別 host の login 不能 2026-09-25 12:10 JST・裁定 2026-09-26T08:28Z「包むが値は 32 GiB」）・ADR-0072（§4 の「cgroup の防壁は席には持ち込まない」の部分 supersede）・SRS v0.25 FR59 / NFR6。
- 現物（verified・main c4d6d20）: 起動行は `crates/scribe2/src/seat/cycle/launch.rs` の `derive_launch`（pure・env 3 語 → `claude` → `--plugin-dir` の列 → `[[launch-arg]]`）で、`launch` が `with_defaults`（役割の既定の `--model` / `--effort`）→ `launch_line`（`cd '<anchor>' && ` の前置）→ `with_tail`（会話の引き継ぎの語）で組み、登録 row の `launch` は導出した行（穴を埋める前）。`cycle::Launch` の構築点は **3 つ**（`crates/scribe2/src/seat/cli.rs` の `seat launch`・`crates/scribe2/src/seat/tick.rs` の `wake`・`crates/scribe2/src/pipe/dispatch/group.rs` の `relaunch`）で関数は 1 本（errata: ADR-0072 と本節の前の版が数えた 4 つ目「follow の起こし直し」は runner の起動〔`pipe/follow.rs` の `spawn` の `Launch`〕で席の起動ではない・grep で実測・run 090333Z の便で判明）。便の封じ込めは `crates/scribe2/src/pipe/confine.rs` の `scope_args`（`--user --scope --quiet --collect --unit=<name> -p MemoryMax=<mb>M -p CPUWeight=<w> -p OOMPolicy=continue`）・`unit_name`（`<NAME>-<場所>-<段>-<n>-<pid>-<seq>`）・`probe`（scope を 1 つ試しに作る）・`Reason`（`no-systemd-run` / `no-scope` / `no-rules` …）。席は §4 のとおり素の行で、user の slice に直に入る。`systemd-run --scope` は `--` の後ろの command を exec して自分は残らない（実測: 子の親は起動した shell＝pane の前面は claude のままで、[seat-heartbeat.md](./seat-heartbeat.md) §6 の pane_is_shell と tick の読みは不変）。env の前置（`A=1 systemd-run … -- claude`）は exec した子へ継承される。e2e の道具箱（`crates/scribe2-boundary/tests/e2e/main.rs` の `toolbox_path`）は偽 `systemd-run`（argv を 1 起動 1 file に写して `--` の後ろを exec）を PATH の先頭に積む。
- 穴: 席の Bash（席自身と subagent）は器の封じ込めの外に在り、1 本の暴走が他の席と ssh の login を道連れにする（NFR6 の「器の子 process」は便だけを指し、席は shell の子なので当たらない＝§4 の判断・本節で覆す）。
- 形（行 t・1 つずつ歯が測る・done と 1:1）:
  1. **席の箱の行**: 埋め込み manifest に rules 行 `seat.memory_max_mb`（kind `SeatMemoryMaxMb`・MiB の整数・値 32768・裁定 id user 2026-09-26T08:28Z）を足す。`RuleKind` の変種は `SeatMoveGraceS` の直後（rows=71 kinds=69 が rules validate の外形と snapshot に載る）。値 0 は「包まない」。行を読めない周（無い・不発効・整数でない）も包まない（語は便の封じ込めと同じ `no-rules`・起動は止めない＝縮退）。
  2. **席の箱の語列は 1 か所**: confine.rs の `scope_args` を「CPUWeight を付けるか」の引数 1 つで 2 形にし（便は付ける・席は付けない＝対話の席を便より軽くしない・他の語と順は同じ）、席の頭は `systemd-run --user --scope --quiet --collect --unit=<name> -p MemoryMax=<mb>M -p OOMPolicy=continue --` の語列（confine.rs の pub な 1 関数・入力 = unit 名と MiB）。unit 名は `unit_name(潰した target, "seat", 0)`（`<NAME>-<target>-seat-0-<pid>-<seq>`・同じ target の起こし直しでも衝突しない）。
  3. **起動行**: `derive_launch` は pure のまま、頭の語列を `Option<&[String]>` で受けて env 3 語の直後・`claude` の前に置く（None なら今の行と 1 字も変わらない）。頭を決めるのは `launch`: `Launch` に欄 1 つ（席の箱の行の読み `Result<u64, RuleRead>`・呼び手の manifest から）を足し、値 0 / 読めない → 無し、`systemd-run` が PATH の dir に実行可能な file として無い → 無し（語 `no-systemd-run`・PATH を探すだけで撃たない）、在れば形 2 の語列。**probe は撃たない**（席の起動は user の session manager が在る host が前提＝tick の unit と同じ・道具箱を積まない歯が実 scope を作る事故〔`s2-07l.504`〕を避ける）。登録 row の `launch`（雛形）には頭を載せない（unit 名は 1 回きり・会話の引き継ぎの語と同じ扱い）。`launch_line` の cd の前置と `with_tail` は不変＝注入する行は `cd '<anchor>' && <env 3 語> systemd-run … -- claude … [-r <sid>]`。
  4. **呼び手 3 つ**（seat launch・tick の `wake`・群の段の `relaunch`）は欄を自分の manifest から埋める（群の段は埋め込み・tick は `--rules` の写しか埋め込み・**`seat launch` は `--rules F` を受ける**〔tick の 4 口と同じ歯の seam・無ければ埋め込み・役割の既定の行の読みも同じ写しから〕＝使い方の 1 行と help の頁の `form` に `[--rules F]` が増える〔run 084705Z の審査の指摘: 埋め込みしか読まない口では `scope=off` / `scope=no-rules` の 2 語を歯が測れない〕）。§4.5 の `account shell` は触らない（役割なしの手動の入口・箱は席の起動の 4 経路）。
  5. **起動の 1 行**: `render_launched` の launched の形（短い形と長い形）の末尾に `scope=<unit 名|off|no-rules|no-systemd-run>` を足す（Refused / None / Failed の周は足さない）。tick の判定行と群の段の記録は不変（起動の結果の語だけ）。event は記さない。
  6. **溢れた周**: kernel が箱の中の 1 process を殺し（`OOMPolicy=continue` で scope は続く）、claude が死んだ周は窓が shell に戻り、[seat-heartbeat.md](./seat-heartbeat.md) §7 の起こし直しが打刻の sid を `--resume` で運ぶ。
- **起動行を読む既存の歯の閉包（census・main 4ee4df8・run 085555Z の審査の指摘）**: 起動行の字面を読む歯は 4 file に在り、頭の有無（PATH に systemd-run が在るか）で動くのは 1 file だけ。(i) `crates/scribe2-boundary/tests/e2e/seat/launch.rs`: 導出行を**全文で pin** する歯が 24 か所（`seat_launch_` 31 本・`seat_entry_` 5 本・`seat_defaults_` 4 本の中）＝この file の起動を撃つ歯は**全部**が期待の行を組む helper 1 つで PATH を固定する（道具箱 = 頭あり・空の dir = 頭なし・固定しない歯を残さない）。(ii) `crates/scribe2-boundary/tests/e2e/seat.rs` の `seat_tick_wake_`: 末尾（`--resume <sid>` と初手）と語の回数だけを読む＝頭の有無で不変。(iii) `crates/scribe2-boundary/tests/e2e/pipe/dispatch.rs` の `launched_lines`（群の段の `relaunch` の受け皿）: 既存の歯は本数と `accounts/<label>` の部分文字列だけを読む＝頭の有無で不変（新しい歯 (i) だけが頭を正に読む）。(iv) `crates/scribe2-boundary/tests/e2e/fleet.rs` の `seat_launch_registration_`: 登録 row の replay で、row の `launch`（雛形）は頭を載せない（形 3）＝不変。(iv) は write-set に入れない（触らない file・頭の有無に依らず GREEN）。(v) **stdout の 1 行の読み手**（run 091124Z の審査の指摘・main ef02653 で grep）: `seat launch: launched …` の字面を読む歯は e2e 全体で 14 か所・全部 (i) の file（母集団 = その字面の grep の全件）＝ `scope=` の末尾は (i) の helper が期待の行に載せる。`render_launched` の呼び手は `crates/scribe2/src/seat/cli.rs` の 4 か所だけ（Refused の 3・起動の結果の 1・全部 write-set の中）で、Refused の行は形 5 のとおり末尾を変えない。`crates/scribe2-boundary/tests/e2e/seat/account.rs` が `seat launch` を撃つ歯は 1 本（群の記録が読めない周の `group-record-unreadable`・rc 1）で launched に達しない＝不変・write-set に入れない。`acct_launch_prefix`（seat.rs の helper・`cd '<anchor>' && <env 2 語> CLAUDE_CONFIG_DIR=` の前置）の呼び手は (i) の file の 1 か所（復元の歯の `starts_with`）だけで seat/account.rs には無い（grep 0 件）＝頭は 3 語目の env（`CLAUDE_CONFIG_DIR=<dir>`）の**後**に来るので前置の `starts_with` は不変。(vi) **(i) の歯は行を実 pane で走らせる**: (i) の起動の歯は偽 tmux でなく本物の tmux（独立 socket・argv を写して実体へ exec する包み）へ注入し、pane の shell が行を**実行**して偽 claude が argv と env を写す。PATH は seat.rs の `launch_shims` が組む 1 本（shim の dir + test の PATH）で、4 つの runner（seat.rs `launch_run`・(i) の file の `launch_run_short` / `launch_group_run` / `launch_default_run`）がそれを引数で受ける。host に本物の systemd-run が在る周は頭が付いて pane が本物の scope を作り（`s2-07l.504` の型）、user の session manager が無い CI では行が落ちる＝ (i) の固定は「道具箱の dir を shim の dir の**前**に積む」を `launch_shims` の 1 か所で行い（積むかを引数 1 つで分ける＝ (i) の helper 1 つ）、道具の無い周を測る歯 (b) だけが systemd-run を含まない PATH（shim の dir だけ・test の PATH の尾を継がない〔包みは実体を絶対 path で exec するので実物の tmux は PATH に要らない〕）で撃ち、行 0 / 行なしを測る歯 (b2) は (c) と同じく**道具箱を積んで**撃つ（道具が在るのに素の行になることを測る＝行 0 で頭を付ける変異は道具箱が在るので落ちる・run 101935Z の審査の指摘: 前の版は (b2) も shim だけの PATH と書き歯 (b2) の「道具箱は積む」と矛盾していた）。(ii) / (iii) の偽 tmux の歯と違い、実 scope を作りうるのはこの file だけ。(vii) **`derive_launch` の呼び手の閉包**: src の呼び手は `crates/scribe2/src/seat/cycle/launch.rs` の `launch` の 1 か所だけ（3 つの構築点は全部 `cycle::launch` を通る＝grep で実測・`relaunch.rs` の `launch_line` は雛形の穴埋めと cd の前置で導出を呼ばない）・同 file の unit 歯 5 か所（全部 None を渡す＝形 3 の「None なら 1 字も変わらない」で不変）・`cycle.rs` の re-export と doc comment の言及（`account/mod.rs`）は呼び手でない。`launch` の中で `prepare` へ渡す row 用の `derived` は None の導出のまま、注入する行だけが頭を持つ（同じ関数を 2 回呼ぶ・呼び手は増えない）。(ii) と (iii) の file には**正の歯**（起こし直しの席にも箱が付く・歯 (c2) / (i)）を足すので write-set に在る（run 090333Z の審査の指摘: 負の周だけでは起こし直しの欄を常に None に固定する変異が生き残る）。host の PATH に本物の systemd-run が在る周も、これらの歯は起動行を偽 tmux へ送るだけで実行せず probe も撃たないので、実 scope は 1 本も作られない（`s2-07l.504` の事故の型に当たらない）。
- 触らない: §4 の口と口座の選定・登録 row の形と書き手・trust の書き（[host-init.md](./host-init.md) §7）・便の封じ込め（[gate-cost.md](./gate-cost.md) §4・`Wrap` / `probe` / `release`）・pane_is_shell・tick の判定の列・host の面・§4.5。
- 却下: 正規表現の字面を見張る（N2・散文の規則）／user の slice の MemoryHigh を下げる（host の運用・器の外・ssh も同じ slice に居る）／席の Bash を hook で 1 command ずつ scope に包む（PreToolUse は command を書き換えられず、subagent の Bash は hook を通らない周が在る・上限が command ごとで席の合計を抑えない）／起動で probe を撃つ（道具箱を積まない歯が実 scope を作る・`s2-07l.504`）／席ごと・役割ごとに値を分ける（rules 行 1 本で足りる・分ける理由が無い）／CPUWeight を席にも付ける（対話の席を便より軽くする理由が無い）／unit 名を target だけにする（前の scope が孤児の process で残る周に衝突して起こせない）。
- 歯（`crates/scribe2-boundary/tests/e2e/seat/launch.rs` の `seat_launch_scope_` 接頭辞・PATH の先頭に道具箱〔`toolbox_path`〕を積む＝偽 systemd-run が実行可能）: (a) 注入した起動行が `cd '<anchor>' && <env 3 語> systemd-run --user --scope --quiet --collect --unit=<NAME>-<潰した target>-seat-0-<pid>-<seq> -p MemoryMax=32768M -p OOMPolicy=continue -- claude …` の形（unit 名は形で pin・`CPUWeight` を含まない）、登録 row の `launch` は素の導出行のまま、起動の 1 行の末尾が `scope=<unit 名>`（base では素の行 ＝ RED）(b) PATH に systemd-run が無い周（shim の dir だけの PATH・census (vi)）は素の行で末尾が `scope=no-systemd-run`・送りは 1 回のまま (b2) `seat launch --rules F` で行の値が 0 の写しは素の行で末尾が `scope=off`、行を欠く写しは素の行で末尾が `scope=no-rules`（各 1 本・道具箱は積む・送りは 1 回のまま・base では `--rules` が使い方の誤り ＝ RED） (c) `crates/scribe2-boundary/tests/e2e/seat.rs` の `seat_tick_wake_` の fixture（`--rules` の写し）で、**PATH の先頭に道具箱を積んだうえで**行を 0 にした周は素の行、行を欠く写しも素の行（起こし直しは止まらない）＝写しを捨てて埋め込み（32768）を読む変異は道具箱が在るので頭が付いて落ちる（run 092619Z の審査の指摘: 道具箱を積まない (c) は systemd-run の無い host で `no-systemd-run` の素の行になり変異を通す） (c2) 同じ fixture で `seat_tick_wake_scope_` 接頭辞の歯: PATH の先頭に道具箱を積み行を **4096**（埋め込みの 32768 と違う値）にした周は、起こし直しの起動行が env 3 語の直後に形 2 の頭（`systemd-run --user --scope` … `-p MemoryMax=4096M -p OOMPolicy=continue --`・unit 名は `<NAME>-<潰した target>-seat-0-<pid>-<seq>` の形）を持ち、末尾の `--resume <sid>` と初手は不変（base では頭が無い ＝ RED・写しを捨てて埋め込みを読む変異は `MemoryMax=32768M` になって落ちる） (i) `crates/scribe2-boundary/tests/e2e/pipe/dispatch.rs` の `pipe_dispatch_group_scope_` 接頭辞の歯: §20 の移動の周の `relaunch` が各 target へ送る起動行（`launched_lines`）が同じ頭を持つ（道具箱は dispatch の fixture が既に積む・base では頭が無い ＝ RED） (d) 既存の `seat_launch_` 31 本・`seat_entry_` 5 本・`seat_defaults_` 4 本（起動を撃つ歯の全部・census の (i)）は道具箱を積んで期待の行に箱の頭を足す（期待の行を組む helper 1 つで足す・`launch_derived` の雛形の pin は素のまま・PATH を固定しない起動の歯を file に残さない）。lib: (e) `seat_launch_derive_line_` の歯が None で不変・Some で env の直後に頭が入ることを pin (f) confine の `confine_scope_args_` 接頭辞の歯が `scope_args` の 2 形（CPUWeight 有り / 無し・他の語と順は同じ）を pin（base では 1 形 ＝ RED）。既存の `seat_launch_` 31 本は接頭辞が fleet.rs の歯の名にも当たるので検証行には置かず、done の 8 門の nextest が全部を撃つ。`crates/scribe2-boundary/tests/e2e/rules.rs`: (g) `rules_seat_box_` 接頭辞の歯が埋め込み manifest の行を pin する: id `seat.memory_max_mb`・kind `SeatMemoryMaxMb`・値 32768・enabled・ruling が `user 2026-09-26T08:28Z`、`RuleKind` の宣言順で `SeatMemoryMaxMb` が `SeatMoveGraceS` の直後（`ALL` の隣接で読む・base では行も variant も無い ＝ RED）、rows=71 kinds=69（`rules_external_form` の snapshot）。(h) 使い方の 1 行に `launch … [--rules F]` が増える（`seat_usage_external_form` の snapshot・help.rs の `form` は同じ 1 行）。
- errata（行 t の実装・`s2-07l.653`）: 歯 (a) / (b) / (b2) を本物の tmux を立てる形で書くと tmux の歯の group（`.config/nextest.toml` の filter）に名を足す必要があり、その file は行 t の write-set の外なので、3 本は §20 の群の外の置き場（偽 tmux が送った行を写すだけで行を走らせない fixture・PATH は偽 tmux の dir → 道具箱）で撃つ。(a) は送った行の全文・unit 名の形・道具箱の偽 systemd-run の記録 0 件（器は systemd-run を 1 度も撃たない＝probe 無し）を測り、(b) の PATH は偽 tmux の dir と、偽 tmux と器が使う道具（`sh` / `cat` / `tr` / `mkdir` / `date`）だけを test の PATH の実体から写した dir（systemd-run を含まない・test の PATH の尾を継がない）。census (vi) の `launch_shims` は引数を持たず常に道具箱を shim の dir の前に積む（本物の tmux を立てる既存の起動の歯は全部が頭付きの行を実 pane で走らせ、偽 systemd-run の記録で頭が走ったことを測る）。`seat launch` が `--rules` を受けるので、`--rules` を受ける席の口を tick だけと数えていた既存の歯（`seat_working_memory_subcommands_are_gone_from_the_usage`）は launch を受け手に数え直す。

## 31. 予約は逼迫した群だけが持つ — 逼迫していない先の群は候補を測らず予約も導かず、逼迫した後の群が鍵の先頭を取る・移り先の無い断りは host の群用 dir の印 `<群>.refused` と doctor の群の行の `refused=` で host の全席から読める（契約表の行 u・§29 形 2 / 3 の改め・ADR-0074〔ADR-0069 の予約の読みの部分 supersede〕・FR38 / AC41・`s2-07l.656`・持ち主の裁定 2026-09-26T14:41Z〔逐語は台帳〕）

やさしく言うと: §29 は「宣言順で先の群（Tier1）が先に移り先を予約し、後の群（Tier2）はそれを避ける」と決め、後の群が移り先なしになるのは想定内とした。ところが先の群は逼迫していなくても予約を持つので、口座 6 つのうち 3 つが上限・2 つが使用中の周に、残る 1 つを暇な Tier1 が予約し、上限に達した Tier2 が 3 時間「移り先なし」で止まった（2026-09-26 12:05Z〜14:07Z・断りは判定した 1 席の画面にしか出ず、人が気づいたのは 14:3xZ）。そこで、予約はその周に逼迫している群だけが持つ形に改める。両方が同時に逼迫した周だけ宣言順（Tier1 が先）が効く。あわせて、移り先の無い断りは群用 dir に印を残し、doctor の群の行に出す（どの席の doctor からも読める）。

- 出所: 台帳 `s2-07l.656`（2026-09-26T14:07Z〜 black4 の Fable 7 日窓 100% で Tier2 が 3 周 `no-candidate`・14:34Z に記録を手で書き換えて 6 席を移した）・持ち主の裁定 2026-09-26T14:41Z（推奨の受諾＝予約は逼迫した群だけ・断りの見える化・§29 の「優先の低い群が移り先なしになるのは想定内」の是認 01:3xZ を覆す・逐語は台帳）・ADR-0074・SRS FR38 / AC41。
- 現物（verified・main 8198b92）: `crates/scribe2/src/hook/group.rs` の `reserve` は群を宣言順に判じる群まで見て、群ごとに門を通る候補（先の群は `head`・判じる群は `taken` を外す・退役中でない・鮮度の外は `measure` で 1 回測る・`fresh_rows` が `pressed` でない）を `by_key` で並べ、先の群の予約でない先頭を予約にする。**先の群の逼迫は読まない**（先の群の今の口座を測らず、候補は全部測る＝§29 形 3「逼迫でない周は候補を測らず予約も導かない」は判じる群にしか効いていない）。判じる群の逼迫は `decide` が今の口座の最新の実測を `pressed` に掛けて確かめてから `reserve` を呼ぶ。移り先が無い周は `refuse` が前の断りより後に今の口座の新しい実測が在る周だけ event `GroupMoveRefused`（detail `group=<名> reason=no-candidate`）を記し、tick は断りの 1 行（`refused_line`）を自席へ、群の段は群の置き場の orchestrator の row（自分の置き場の分）へ送る。doctor の群の行は `render_group`（`group= accounts= anchors= seat-accounts= current= next=`）で、断りの有無は出ない。群用 dir の file は `<群>.account`（記録）・`<群>.request`（頼み）・`<群>.judged`（判定の打刻）の 3 種（`group_file`）。
- 穴（verified）: (1) 逼迫していない先の群が予約を持ち、逼迫した後の群の唯一の候補を奪う（2026-09-26 12:05Z〜14:07Z の Tier2: black1 95% / black2 100% / black6 100% は閾値以上・black3 は Tier1 の今の口座・black5 82% は Tier1 の予約→ none）。(2) 断りは判定した 1 席の pane と event log にしか出ず、host の他の席（別の置き場の tick は別の state dir の event を読む）からは見えない。
- 形（行 u・1 つずつ歯が測る・done と 1:1）:
  1. **予約は逼迫した群だけ**: `reserve` は先の群 h ごとに、まず h の今の口座（`current_of` の label・記録を読めない群は予約なし）の鮮度の内側の実測を読み（鮮度の外なら `measure` で **今の口座だけ** 1 回測る・口座ごとに 1 周 1 回の集合は今のまま）、`pressed` の周だけ h の候補を測って予約を導く。逼迫でない h・実測を持たない h は予約なし・候補を測らない。判じる群は今のまま（`decide` が逼迫を確かめてから呼ぶ）。両方が逼迫した周は宣言順（Tier1 が鍵の先頭・Tier2 は次＝§29 形 2 の残り）。役割の行が無い周の `NoRule` は不変。
  2. **断りの印は 1 file**: `refuse` が event を記す周（`Refusal::Recorded`）に群用 dir に `<群>.refused`（`group_file` の 4 種目・1 行 `ts=<UTC の秒> reason=no-candidate`・一時 file → rename・書き手は hook/group.rs の 1 関数）を書く。`Repeated` の周は書き直さない。`decide` が `Stay` を返す周と `approve` が記録を書く周は印が在れば `to_history` へ退避（消さない・C1）。読み手は doctor だけ（判定は読まない）。
  3. **doctor の群の行に `refused=<ts|-|unreadable>`** を `next=` の後ろに足す（印が在ればその ts・無ければ `-`・在るのに形でなければ `unreadable`・群 0 の host は 1 語も変わらない）。
  4. **触らない**: 残量の鍵（§29 形 1）・門・判じる群の予約の導き・tick の断りの宛先（自席）と群の段の断りの宛先・断りの event の 1 回性・`next=` の語・移動の契機と lock・記録の形・種（§28）・便用の選定（§23）。
...
## 33. 逼迫の門のモデル別窓は役割の model の窓だけ — 5 時間窓と 7 日窓は今のまま全部、モデル別 7 日窓は席の役割の model の行だけを数える（契約表の行 w・§19 / §29 の門の改め・ADR-0075〔ADR-0055 / ADR-0069 の窓の読みの部分 supersede〕・FR38 / AC41・`s2-07l.663`・持ち主の裁定 2026-09-26T15:52Z〔逐語は台帳〕）

やさしく言うと: 口座が逼迫かどうかは 3 つの窓（5 時間・7 日・model ごとの 7 日）で判じ、どれかが閾値以上なら逼迫とする。今は model ごとの窓を「どの model でも」数えるので、席が Opus で走るようになっても、Fable の窓が埋まった口座は逼迫と読まれ、移り先にも使えず、今の口座なら移動を起こす。usage は Fable の窓しか返していないので、席を Opus に替えた意味が群の判定に届かない。そこで、model ごとの窓は「席の役割の model」の窓だけを数える形に改める。5 時間窓と 7 日窓（全 model の合計）は今のまま全部数える。

- 出所: 台帳 `s2-07l.663`（2026-09-26 の実測: 口座 6 つのうち 5 つの Fable の窓が 95 以上・うち 4 つは 7 日窓が 50〜92・口座名は台帳）・持ち主の裁定 2026-09-26T15:52Z（推奨の受諾・逐語は台帳）・契約 `s2-07l.662`（orchestrator の既定を opus / xhigh に・seat-roles.md §28）・ADR-0075。
- 現物（verified・main 0d32054）: 逼迫の判定は `crates/scribe2/src/hook/group.rs` の `pressed`（pure・口座の最新の実測の行のうち `Measured` の各行を窓の閾値〔`Caps`〕と比べ、どれかが閾値以上なら越えた窓の最大の 1 つ）で、モデル別窓の行は `model` を見ずに数える。呼び手は 5 つ: 同 file の `decide`（群の今の口座と席の口座の逼迫）・`reserve`（候補の門）・`pressed_now`（§31 の先の群の今の口座）・席の turn の hook の逼迫の 1 行（`put_request` の手前）と、`crates/scribe2/src/seat/tick.rs` の `account_pressed`（管理 tick の合図の口座の門・自席の口座）。役割の model の集合は同 file の `role_models`（群の置き場の席の登録 row の役割ごとの rules 行 `seat.model.<役割>` の表示名・行が無い周は `None`・席の row の無い群は空）で、今は残量の鍵（`by_key`）にだけ使う。tick の `front` は自席の登録 row の役割を読む（`registration_of_target`）。
- 穴（verified）: 席の役割の model が Fable でない周も Fable の窓が門を閉じる。Fable の窓が埋まり 7 日窓に余裕のある口座（上の 4 つ）は、Opus の席の群の移り先にならず、今の口座なら逼迫と判じられて移動や断りを起こす。
- 形（行 w・1 つずつ歯が測る・done と 1:1）:
  1. **門の窓は 1 関数**: `pressed` に役割の model の表示名の集合（`&BTreeSet<&str>`）を渡す引数を 1 つ足し、モデル別 7 日窓の行は `model` の表示名（`Model::parse` → `display`）が集合に在る行だけを数える。5 時間窓と 7 日窓は集合に依らず今のまま数える。集合が空の周（席の row の無い群・役割の行を読めない tick）はモデル別窓を数えない（`by_key` の空の集合と同じ読み）。`model` を parse できない行は数えない。
  2. **役割 1 つの model は 1 関数**: `role_models` の中の「役割 → rules 行 `seat.model.<役割>` の表示名」を pub な 1 関数（入力 = manifest・役割）に切り出し、`role_models` はそれを登録 row ごとに呼ぶ（読みは 1 本・C2）。
  3. **呼び手 5 つ**: `decide` は判じる群の `role_models` を今の口座の逼迫を測る前に読み、`None` の周は `Judgement::Unreadable`（§29 の `NoRule` と同じ止まり方＝記録 0・event 0・今は逼迫の周だけがそこへ落ちる）。`reserve` は群ごとに既に持つ集合を候補の門と `pressed_now` の両方へ渡す（`pressed_now` は集合を引数に受け、自分では読まない）。席の turn の hook は判じる群の `role_models` を渡し、`None` の周は 1 行も出さない。tick は `Front` に登録 row の役割を持たせ、`account_pressed` は自席の役割 1 つの集合（形 2 の関数・行が無い周は空＝門は正の証拠でだけ閉じる）を渡す。役割の表は今 `orchestrator` の 1 つで、埋め込みの面はその行を持つ（`None` は `--rules` の写しが行を欠く周だけ）。
  4. **触らない**: 閾値の rules 行 3 本と値・`Caps`・残量の鍵・予約の規則（§29 / §31）・便用の口座選定（ADR-0042・`runner.model` の窓・`fleet/select.rs`）・usage の計測と実測の行の形・event の種類。
...
depends = ["s"]
done = "(1) 残量の鍵は pure な 1 関数で、鮮度の内側の実測の行から min(100 − 7 日窓の使用率, 役割の model ごとの 100 − モデル別 7 日窓の使用率) の大きい順 → 7 日窓の resets_at の早い順（無い口座は後）→ 宣言の候補の index の辞書順に並べ、役割の model の集合（群の置き場の席の登録 row の役割ごとの rules 行 seat.model.<役割> の表示名・重複は畳む）のどれか 1 つでもモデル別窓の行を持たない口座は最後、集合が空の群（席の row が無い）は 7 日窓だけで並べ、役割の行が無い周は集合を空に読み替えず typed に止まり（1 周の群の段は移らず記録 0・承認 event 0・断りの event 0・便の列の rc 不変、tick は judged=error、doctor の next= は no-rule）、5 時間窓は門にだけ使う (2) 群の予約は 1 関数で、群を宣言順に見て門（今の口座でなく・どの群の今の口座でもなく・退役中でなく・鮮度の内側の実測を持ち 3 窓とも閾値未満・鮮度の外は 1 回測る）を通る候補を鍵で並べたものから先の群の予約でない先頭を返し、移る群は自分の予約へ移り、宣言順で最初に門を通る label は返さない (3) 1 周の群の段と tick の判定は lock の内側で逼迫した群を判じる周にだけ、その群と前の群の予約を導き（前の群の候補も鮮度の外は測る・口座ごとに 1 周 1 回）、逼迫でない周は候補を測らず、群用 dir に予約の file は 0 (4) §23 の便用の除外と便用の並べ鍵は 1 字も変わらず、予約の立つ口座を便は使う (5) doctor の群の行に next=<label|none|unreadable|no-rule> が current= の後ろに増え、同じ予約の関数を測らずに呼び、群 0 の host は 1 語も変わらない (6) 移動の契機・lock・記録・承認 event・退避・起こし直し・閾値の行・event の種類は不変 歯: pipe_dispatch_group_reserve_ が残量の逆順の 2 候補で後の候補へ（base では先 ＝ RED）・model の窓の残量で決まる 2 候補・同点は reset の早い方と宣言順・model の行を欠く候補は最後・5 時間窓は門だけ（2 本）・2 群同時の逼迫で Tier1 が鍵の先頭・Tier2 だけの逼迫でも Tier1 の予約を飛ばす（base では取る ＝ RED）・同じ周の便用の選定が予約の口座を使う・Tier2 だけの逼迫で Tier1 の候補が測られる・役割の行を欠く --rules の逼迫の周は移らず記録 0・event 0・rc 不変（base では移る ＝ RED） を測り、seat_tick_judge_reserve_ が tick の判定も Tier1 の予約を飛ばすことと役割の行を欠く --rules で judged=error・0 key を、host_group_next_ が next= の label / none / unreadable / no-rule を測り、既存の host_group_doctor_line_says_none_without_seat_rows は next= の増分で書き換えて GREEN、lib の group_key_ が並びを行の値だけで測る"
...
verify = ["cargo nextest run -p scribe2-boundary --test e2e --no-tests=fail host_group_tier_", "cargo nextest run -p scribe2-boundary --test e2e --no-tests=fail host_group_", "cargo nextest run -p scribe2-boundary --test e2e --no-tests=fail rules_review_same_", "cargo nextest run -p scribe2-boundary --test e2e --no-tests=fail pipe_dispatch_group_", "cargo nextest run -p scribe2-boundary --test e2e --no-tests=fail seat_tick_", "cargo nextest run -p scribe2-boundary --test e2e --no-tests=fail seat_doctor_run_", "cargo nextest run -p scribe2-boundary --test e2e --no-tests=fail seat_pane_shell_", "cargo nextest run -p scribe2-boundary --test e2e --no-tests=fail seat_heartbeat_", "cargo nextest run -p scribe2-boundary --test e2e --no-tests=fail seat_launch_", "cargo nextest run -p scribe2-boundary --test e2e --no-tests=fail hook_group_", "cargo nextest run -p scribe2-boundary --test e2e --no-tests=fail init_repo_", "cargo nextest run -p scribe2-boundary --test e2e --no-tests=fail init_seat_", "cargo nextest run -p scribe2-boundary --test e2e --no-tests=fail host_init_"]
size = "S"
growth = ["crates/scribe2/src/rules/manifest.rs:3", "crates/scribe2/src/rules/mod.rs:3", "crates/scribe2/src/rules/groups.rs:80"]
done = "(1) 群の名は Tier の後ろに 10 進の数字 1 桁以上（先頭の 0 は不可）に限り、外れる名は群の見出し行の欠陥（check_duplicate_groups と同じ列 Vec<RuleError> に RuleError の new で 1 件・文は語「Tier と数字の形でない」を含む・enum も struct も足さない・検査の本体は write-set の + の file〔rules の兄弟 module・mod.rs に mod 宣言 1 行〕に在り manifest.rs に増えるのは呼び出し 1 行だけ） (2) 宣言順は数字の狭義の昇順で、前の群の数字以下の群は欠陥（文は語「前の群より大きくない」を含む・数値で比べる）、名が前の群と同じ周は昇順の欠陥を重ねず名の重複の 1 件だけ（既存の歯の件数の pin は名の改めだけで GREEN）、名の重複の検査は残る (3) 読めない面は全群を typed に止める今の規則のまま、優先は宣言順のままで数字は並べ替えに使わない (4) e2e 8 file の fixture の群の名を Tier と数字に改め、assert の他の語は変えず、群の fixture に到達する既存の歯 131 本（§29 の census・main 66e0089・検証行の接頭辞 13 本が全部を撃つ）は GREEN で、改名だけの 7 file には file ごとに札 flip-check: retroactive <本 bead> を 1 行置き（4 条件・rules.rs は新しい歯が base で赤なので札なし）、変異 A/B（Tier でない名を通す・数字を辞書順で比べる）の撃墜と母集団を notes に残す 歯: host_group_tier_ が alpha の群の欠陥（行番号は群の行・base では通る ＝ RED）・Tier2, Tier1 の順の 2 番目の行の欠陥・Tier1, Tier1 の名の重複の 1 件だけ・Tier01 の形の欠陥・Tier1, Tier2, Tier10 は通る を測る"
[[contract]]
id = "t"
title = "席の起動を封じ込めの箱で包む — rules 行 seat.memory_max_mb（32768）・起動行の頭の systemd-run --scope（unit 名は便と同じ 1 関数・CPUWeight 無し）・行が 0 / 読めない周と道具が PATH に無い host は素の行・起動の 1 行の scope=（§30・ADR-0072・s2-07l.627）"
req = ["FR59", "NFR6", "FR27", "NFR4"]
section = "30"
...
verify = ["cargo nextest run -p scribe2-boundary --test e2e --no-tests=fail seat_launch_scope_", "cargo nextest run -p scribe2-boundary --test e2e --no-tests=fail seat_tick_wake_", "cargo nextest run -p scribe2-boundary --test e2e --no-tests=fail pipe_dispatch_group_scope_", "cargo nextest run -p scribe2-boundary --test e2e --no-tests=fail rules_seat_box_", "cargo nextest run -p scribe2-boundary --test e2e --no-tests=fail rules_external_form", "cargo nextest run -p scribe2-boundary --test e2e --no-tests=fail seat_usage_external_form", "cargo nextest run -p scribe2-boundary --test e2e --no-tests=fail rules_embedded_manifest_is_valid_and_covers_all_kinds", "cargo nextest run -p scribe2-boundary --test e2e --no-tests=fail rules_embedded_manifest_declares_host_guard_kinds_at_the_tail_of_all", "cargo nextest run -p scribe2 --lib --no-tests=fail seat_launch_derive_line_", "cargo nextest run -p scribe2 --lib --no-tests=fail confine_scope_args_"]
size = "M"
growth = ["crates/scribe2/src/rules/mod.rs:8", "crates/scribe2/src/seat/cycle/launch.rs:50", "crates/scribe2/src/pipe/confine.rs:40", "crates/scribe2/src/seat/cli.rs:24", "crates/scribe2/src/help.rs:4", "crates/scribe2/src/seat/tick.rs:6", "crates/scribe2/src/pipe/dispatch/group.rs:6"]
done = "(1) 埋め込み manifest に seat.memory_max_mb（kind SeatMemoryMaxMb・値 32768・裁定 id user 2026-09-26T08:28Z）が SeatMoveGraceS の直後の宣言順で在り（rules_seat_box_ の歯が id / kind / 値 / 裁定 id / 隣接を pin）rows=71 kinds=69 が rules validate の外形と snapshot に載る (2) confine.rs の scope_args が CPUWeight の有無の引数で 2 形になり、席の頭の語列は pub な 1 関数（unit 名と MiB）で systemd-run --user --scope --quiet --collect --unit=<name> -p MemoryMax=<mb>M -p OOMPolicy=continue -- の順・unit 名は unit_name(潰した target, seat, 0) (3) derive_launch が頭の語列を Option で受け env 3 語の直後に置き、None は 1 字も変わらず、Launch の欄 1 つ（行の読み）を 3 つの構築点（seat launch・tick の wake・群の段の relaunch）が自分の manifest から埋め（seat launch は --rules F の写し・無ければ埋め込み・使い方の 1 行と help の form に [--rules F] が増える）、launch は値 0 / 読めない周と PATH に実行可能な systemd-run が無い周は頭を付けず（probe は撃たない）、登録 row の launch は素の導出行のまま、cd の前置と会話の引き継ぎの語は不変 (4) render_launched の launched の 2 形の末尾に scope=<unit 名|off|no-rules|no-systemd-run> が付き Refused / None / Failed には付かず、tick の判定行と群の段の記録と event は不変 (5) §4 の口・口座の選定・登録 row・trust・便の封じ込めの Wrap / probe / release・pane_is_shell・§4.5 は不変で、seat_launch_scope_（道具箱を積んだ起動行の形と unit 名の形と scope=<unit>・道具箱なし〔shim だけの PATH〕の素の行と scope=no-systemd-run・道具箱つきで --rules の行 0 の素の行と scope=off・道具箱つきで行を欠く写しの素の行と scope=no-rules）・seat_tick_wake_ の道具箱つきで --rules の 0 と行なしの素の行・seat_tick_wake_scope_ の道具箱 + 埋め込みと違う値 4096 で頭付きの起こし直しの行（MemoryMax=4096M）・pipe_dispatch_group_scope_ の群の段の relaunch の頭付きの行・既存 seat_launch_ 31 本と seat_entry_ 5 本と seat_defaults_ 4 本の期待の行の頭（seat/launch.rs の起動の歯は全部 PATH を固定・seat.rs / dispatch.rs / fleet.rs の既存の読み手は頭に依らず不変）・lib の derive_line と scope_args の 2 形・rules_seat_box_ の行の pin と順と snapshot・usage の snapshot を測る"
[[contract]]
id = "u"
title = "予約は逼迫した群だけが持つ — 逼迫していない先の群は候補を測らず予約も導かず、逼迫した後の群が鍵の先頭を取る・移り先の無い断りは群用 dir の印 <群>.refused と doctor の群の行の refused= で読める（§31・ADR-0074・s2-07l.656・裁定 2026-09-26T14:41Z）"
req = ["FR38", "FR27", "AC41", "NFR4"]
section = "31"
write-set = ["crates/scribe2/src/hook/group.rs", "crates/scribe2/src/account/mod.rs", "crates/scribe2-boundary/tests/e2e/pipe/dispatch.rs", "crates/scribe2-boundary/tests/e2e/seat.rs", "crates/scribe2-boundary/tests/e2e/seat/account.rs", "docs/design/account-lifecycle.md"]
verify = ["cargo nextest run -p scribe2-boundary --test e2e --no-tests=fail pipe_dispatch_group_reserve_", "cargo nextest run -p scribe2-boundary --test e2e --no-tests=fail seat_tick_judge_reserve_", "cargo nextest run -p scribe2-boundary --test e2e --no-tests=fail host_group_refused_", "cargo nextest run -p scribe2-boundary --test e2e --no-tests=fail host_group_next_", "cargo nextest run -p scribe2-boundary --test e2e --no-tests=fail host_group_doctor_", "cargo nextest run -p scribe2 --lib --no-tests=fail group_refused_"]
size = "M"
...
[[contract]]
id = "w"
title = "逼迫の門のモデル別窓は役割の model の窓だけ — pressed に役割の model の表示名の集合を渡し、モデル別 7 日窓は集合に在る model の行だけを数える（5 時間窓と 7 日窓は今のまま）・役割 1 つの model を読む 1 関数・呼び手 5 つ（§33・ADR-0075・s2-07l.663・裁定 2026-09-26T15:52Z）"
req = ["FR38", "FR27", "AC41", "NFR4"]
section = "33"
write-set = ["crates/scribe2/src/hook/group.rs", "crates/scribe2/src/seat/tick.rs", "crates/scribe2-boundary/tests/e2e/pipe/dispatch.rs", "crates/scribe2-boundary/tests/e2e/seat.rs", "crates/scribe2-boundary/tests/e2e/seat/account.rs", "crates/scribe2-boundary/tests/e2e/hook.rs", "docs/design/account-lifecycle.md"]
verify = ["cargo nextest run -p scribe2-boundary --test e2e --no-tests=fail pipe_dispatch_group_model_gate_", "cargo nextest run -p scribe2-boundary --test e2e --no-tests=fail seat_tick_judge_model_gate_", "cargo nextest run -p scribe2-boundary --test e2e --no-tests=fail host_group_next_model_gate_", "cargo nextest run -p scribe2-boundary --test e2e --no-tests=fail hook_group_model_gate_", "cargo nextest run -p scribe2-boundary --test e2e --no-tests=fail hook_group_model_window_alone_prints_the_model_window", "cargo nextest run -p scribe2 --lib --no-tests=fail hook_group_pressed_"]
size = "M"
growth = ["crates/scribe2/src/hook/group.rs:40", "crates/scribe2/src/seat/tick.rs:10"]
done = "(1) pressed は役割の model の表示名の集合を受け、モデル別 7 日窓は model の表示名が集合に在る行だけを数え、集合が空ならモデル別窓を数えず、5 時間窓と 7 日窓は集合に依らない (2) 役割 1 つの model を読む pub な 1 関数が在り role_models はそれを登録 row ごとに呼ぶ (3) decide は判じる群の role_models を逼迫を測る前に読み None の周は Unreadable、reserve は群ごとの集合を候補の門と pressed_now へ、席の turn の hook は判じる群の role_models を（None の周は 0 行）、tick は Front の役割から account_pressed へ自席の役割 1 つの集合（行が無い周は空）を渡す (4) 閾値の行・Caps・残量の鍵・予約の規則・便用の口座選定・usage の計測は不変で、pipe_dispatch_group_model_gate_（役割 opus で Fable の窓だけ高い今の口座は移らない・その候補へ移れる・役割 fable は不変）・seat_tick_judge_model_gate_・host_group_next_model_gate_・hook_group_model_gate_ と put_group_round の窓の名を埋め込みの役割の既定に揃えた既存の歯・lib の hook_group_pressed_ を測る"
<!-- contracts:end -->
