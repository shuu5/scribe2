...
- 形（行 t・1 つずつ歯が測る・done と 1:1）:
  ...
  4. **呼び手 4 つ**（seat launch・tick の `wake`・群の段の `relaunch`・follow の起こし直し）は欄を自分の manifest から埋める（seat launch と follow と群の段は埋め込み・tick は `--rules` の写しか埋め込み＝歯の seam）。§4.5 の `account shell` は触らない（役割なしの手動の入口・箱は席の起動の 4 経路）。
...
size = "M"
growth = ["crates/scribe2/src/rules/mod.rs:8", "crates/scribe2/src/seat/cycle/launch.rs:50", "crates/scribe2/src/pipe/confine.rs:40", "crates/scribe2/src/seat/cli.rs:6", "crates/scribe2/src/seat/tick.rs:6", "crates/scribe2/src/pipe/dispatch/group.rs:6", "crates/scribe2/src/pipe/follow.rs:6"]
done = "(1) 埋め込み manifest に seat.memory_max_mb（kind SeatMemoryMaxMb・値 32768・裁定 id user 2026-09-26T08:28Z）が SeatMoveGraceS の直後の宣言順で在り rows=71 kinds=69 が rules validate の外形と snapshot に載る (2) confine.rs の scope_args が CPUWeight の有無の引数で 2 形になり、席の頭の語列は pub な 1 関数（unit 名と MiB）で systemd-run --user --scope --quiet --collect --unit=<name> -p MemoryMax=<mb>M -p OOMPolicy=continue -- の順・unit 名は unit_name(潰した target, seat, 0) (3) derive_launch が頭の語列を Option で受け env 3 語の直後に置き、None は 1 字も変わらず、Launch の欄 1 つ（行の読み）を 4 つの構築点が自分の manifest から埋め、launch は値 0 / 読めない周と PATH に実行可能な systemd-run が無い周は頭を付けず（probe は撃たない）、登録 row の launch は素の導出行のまま、cd の前置と会話の引き継ぎの語は不変 (4) render_launched の launched の 2 形の末尾に scope=<unit 名|off|no-rules|no-systemd-run> が付き Refused / None / Failed には付かず、tick の判定行と群の段の記録と event は不変 (5) §4 の口・口座の選定・登録 row・trust・便の封じ込めの Wrap / probe / release・pane_is_shell・§4.5 は不変で、seat_launch_scope_（道具箱を積んだ起動行の形と unit 名の形と scope=<unit>・道具箱なしの素の行と scope=no-systemd-run）・seat_tick_wake_ の --rules で 0 と行なしの素の行・既存 seat_launch_ 31 本の期待の行の頭・lib の derive_line と scope_args の 2 形・rules の行と順と snapshot を測る"
<!-- contracts:end -->
