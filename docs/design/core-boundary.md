...
| 実物 1 つを境界 crate の 1 file に持ち bin の main の先頭で据え、std の Command の構築はこの file だけ | 3・9 | c・i | c の done (3)（受け入れの時点）・i の done (1)（core 側の deny）・i の done (2)（境界 crate 側の deny） |
