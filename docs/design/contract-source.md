...
- **write-set の導出（契約 (h)・user 指摘 2026-09-14「恒久的に write-set が適切に分配される質の高い計画を書けるような修正」）**: write-set を planner が手で列挙する形は、不足（閉包の file が無い＝(g) が断る）と余分（触らない file を挙げて別の便と偽の交差を起こす＝直列化の根）の両方を人の注意に頼る。行は **何を触るか（`touches`）・何で測るか（`verify`）・何を新設するか（`creates`）・Rust の外で何を触るか（`also`）** だけを宣言し、write-set は器が **導出値** として作る。導出 = 次の和集合（各項は 1 関数・宣言順・`pipe/closure.rs`）: (i) 閉包 = `touches` の型を構造として持つ `.rs`（上の 4 形）(ii) **歯の置き場** = `verify` 行のうち `cargo nextest run` の行の末尾の filter 語（`<module>_<題>_…` の形）から `crates/<NAME>/tests/e2e/<module>/<題>.rs`（base に在れば）→ 無ければ `crates/<NAME>/tests/e2e/<module>.rs`（base に在れば）→ どちらも無ければ `Refuse::TeethPlaceUnresolved { filter }`（新しい module の歯は `creates` で宣言する）(iii) 外形 = `surfaces` の snapshot と pin の file（上の第 5 形）(iv) `creates` = 新規 file の列（`+` 接頭辞と同じ意味・base に無いことを検査）(v) `also` = base に実在する **非 `.rs`** の file（`rules/manifest.toml`・`.github/workflows/ci.yml`・`docs/design/*.md`・`contracts/schema.toml`）。`also` に `.rs` を書いた行は `Refuse::AlsoNamesRust { item }`（Rust の面は `touches` から導く＝「どの型を触るか」を宣言させる側に倒す）。
- **手書きの write-set の扱い**: 行に `write-set` が在れば導出値と**集合として一致**しなければ断る（`Refuse::WriteSetDrift { missing, extra }`・不足も余分も全部名指す）。一致する手書きは冗長なだけで害が無い（移行期の既存行はそのまま通る）。`write-set` の無い行は導出値を write-set とし、intake が生成する契約 file（(b)）と runner の allowlist に書く（手書きを写す口は無くなる）。契約 file を直接渡す形（`--contract`）でも同じ照合を撃つ（(b) の land まで admin が写す契約 file が導出値と食い違えば断る）。
- **限界（残す側）**: 歯の置き場は filter 語の先頭 2 語で決める規約に依り、規約の外の名（接頭辞が module 名で始まらない歯）は (ii) で解けず `creates` / 既存 file 名の宣言に倒す。閉包の下界（別名・generic）は上と同じ。導出値は intake の判定行に `write-set=<導出|手書き一致> files=<N>` として出す（何が導出されたかを人が読める・C10 の派生値）。
...
[[contract]]
id = "h"
title = "write-set の導出 — touches の閉包 + verify の歯の置き場 + surfaces + creates / also から器が write-set を作り、手書きの write-set は導出値との集合一致だけを認める"
req = ["FR48", "FR39", "FR47"]
section = "3"
touches = ["crate::pipe::refuse::Refuse"]
write-set = ["crates/scribe2/src/pipe/closure.rs", "crates/scribe2/src/pipe/refuse.rs", "crates/scribe2/src/pipe/declaration.rs", "crates/scribe2/src/pipe/table.rs", "crates/scribe2/src/pipe/cli.rs", "contracts/schema.toml", "crates/scribe2/tests/e2e/pipe.rs"]
...
size = "M"
done = "write-set 無しの行が導出値で intake を通り契約 file に導出値が載る・手書きが導出値とずれた行は missing / extra を名指して断られる・also の .rs と解けない filter は typed に断られる・現物の契約表（全 tracked doc）で drift 0"
depends = ["g"]
<!-- contracts:end -->
