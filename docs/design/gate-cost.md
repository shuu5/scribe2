...
- 歯（置き場は既存の file・新設の test file は作らない）:
  ...
  - **行 an**（(j)(n) は `tests/e2e/pipe/gate.rs`・接頭辞 `pipe_detection_off_gate_`・base では ③ を撃つ ＝ RED）: (j) 検出線を宣言した便の gate は stub を 1 回も呼ばず、`verify.jsonl` に `kind=detection` の record が無く、land の後に主実測の record に `landed` の無い `kind=detection` が 0 本（base では ③ を撃つ ＝ RED）。(n) main が `crates/` に触れて動いた便の追随の再 gate も stub を呼ばない。docs だけの動きは再 gate を省く（pipeline.md §33 の既存の歯が緑のまま）。消す歯 31 本・直す歯 6 本・残す歯 25 本は形 (13) の数え上げのとおりで、`pipe_record_show_external_form` の snapshot は検出線の判定行の無い形で更新する。行 an の diff の見込みは約 140 KB（.589 の実測の内訳から消す歯 ≈ 90 KB + src ≈ 45 KB）で cap 150000 byte に近い＝超えた周は行 an を「gate から外す + gate の歯」と「主実測・候補の木から外す + 型を消す + 残りの歯」の 2 便に割る。
...
[[contract]]
id = "an"
title = "検出線を gate と主実測と候補の木から外し ③ を撃つことを測る既存の歯 31 本を消し 6 本を直す（行 am の後・形 (9)(10)(13)・Detection と gate の材料の欄と DetectionSkip::SameTree を消し pipe show の snapshot を更新）"
req = ["FR8", "NFR6", "FR50"]
section = "44"
write-set = ["crates/scribe2/src/pipe/gate.rs", "crates/scribe2/src/pipe/gate/verify.rs", "crates/scribe2/src/pipe/gate/record.rs", "crates/scribe2/src/pipe/land.rs", "crates/scribe2/src/pipe/land/verify.rs", "crates/scribe2/src/pipe/train.rs", "crates/scribe2/src/pipe/cli/step.rs", "crates/scribe2-boundary/tests/e2e/pipe/gate.rs", "crates/scribe2-boundary/tests/e2e/pipe/land.rs", "crates/scribe2-boundary/tests/e2e/snapshots/e2e__pipe__gate__pipe_record_show_external_form.snap", "docs/design/gate-cost.md"]
...
size = "M"
depends = ["am"]
done = "(9) gate・主実測・候補の木の段の列に ③ が無く（gate と主実測は ①②④）、Detection と gate の材料の欄と主実測の面の判定と DetectionSkip の SameTree と候補の木の後続ごとの検出線の段が無く（site の数え上げは §44 の形 9・main c135769・7 file 61 site・cli/step.rs の Gate の構築点を含む）、同じ木の主実測の record 1 本と木が違う周の ①②④ は変わらない (10) 追随の面の判定が再 gate を省くかだけを返し、path を読めない周は撃ち直す側のまま (13) ③ を gate と主実測と候補の木が撃つことを測る既存の歯が消え（消す 31 本・直す 6 本・残す 25 本の数え上げは §44 の形 13・main df06c83）、同じ木の主実測の record 1 本の不変は pipe_main_same_tree_writes_one_record_and_fires_nothing が残って測り、③ を宣言した toy を land まで通す残りの歯は子の終わりを待ってから測って緑、pipe show の外形の snapshot は検出線の判定行の無い形で更新され、tests/e2e/pipe.rs の stub の定義と cli/show.rs は触らない 歯: pipe_detection_off_gate_ の歯が、検出線を宣言した便の gate が stub を 1 回も呼ばず verify.jsonl に kind=detection が無く land の後の主実測の record に landed の無い kind=detection が 0 本・main が crates に触れて動いた追随の再 gate も stub を呼ばない、を測る"
