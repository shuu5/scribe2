...
- 歯（置き場は既存の file・新設の test file は作らない）:
  ...
  - **行 ao**（(p) は `tests/e2e/pipe/gate.rs`・接頭辞 `pipe_detection_off_main_`・base では主実測が ③ を撃つ ＝ RED）: (p) 検出線を宣言した便を land まで通すと、主実測の record に `landed` の無い `kind=detection` が 0 本で（木が違う周も ①②④ だけ）、候補の木の後続の record も同じ（base では主実測が ③ を撃つか `skipped=detection` を書く ＝ RED）。同じ木の周は record 1 本のまま（`pipe_main_same_tree_writes_one_record_and_fires_nothing` が残って測る）。消す歯 8 本と直す歯 2 本は形 (13) の便の割り当てのとおり。型 `Detection` と `DetectionSkip::SameTree` と `Gate` の欄を消し、追随の面の判定を形 (10) の 1 役にする。
