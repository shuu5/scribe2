...
size = "M"
depends = ["am"]
done = "(9a) gate の段の列に ③ が無く（gate は ①②④・Detection::Run の周も撃たず写しも書かない・型 Detection と DetectionSkip と Gate の欄と cli/step.rs と land.rs の構築点は不変）、主実測と候補の木の段は変わらない (13a) ③ を gate が撃つことを測る既存の歯が消え（消す 23 本と直す 4 本は §44 の形 13 の便の割り当て・main d11b83f）、pipe show の外形の snapshot は検出線の判定行の無い形で更新され、tests/e2e/pipe.rs の stub の定義と cli/show.rs は触らない 歯: pipe_detection_off_gate_ の歯が、検出線を宣言した便の gate が stub を 1 回も呼ばず verify.jsonl に kind=detection が無く pipe show に判定行が無い・main が crates に触れて動いた追随の再 gate も stub を呼ばない、を測る"
