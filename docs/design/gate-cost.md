...
- 形（番号は done と 1:1）:
  ...
  - **行 an（③ を gate と主実測と候補の木から外す・行 am の後）**
    9. **外す**: gate・主実測・候補の木の段の列から ③ を外す（gate と主実測の段は ①②④・③ は行 ak の入口だけが撃つ）。`Detection` と gate の材料の欄、主実測の面の判定と `DetectionSkip::SameTree`、候補の木の後続ごとの検出線の段を消す。同じ木の主実測の record 1 本（§27・ADR-0043）と、木が違う周の ①②④ は変えない。
