...
- 形（番号は done と 1:1）:
  ...
  - **行 an（③ を gate と主実測と候補の木から外す・行 am の後）**
    ...
    13. **歯の置き換え**: ③ を gate と主実測と候補の木が撃つことを測る既存の歯（写し・{teeth}・的・純移動の母集団・段の順・面の判定・③ の箱の死・段の秒）を消す。同じ性質は行 ak の歯 (a)〜(f) が口の上で測る。③ を宣言した toy を land まで通す残りの歯（§27 の同じ木の歯など）は、行 ak の helper（下の「実装の決め」(i)）で子の終わりを待ってから測る。**数え上げ（実測・main bc88844・検出線の stub か detection の字面に触れる fn）**: `tests/e2e/pipe/gate.rs` は fn 263 本中 76 本（歯 46・fixture の helper 30）、`tests/e2e/pipe/land.rs` は fn 218 本中 7 本（歯 4・helper 3）で、全部が行 an の write-set の中。歯 50 本の内訳（接頭辞・本数）: 消す側 = `pipe_gate_detection_` 8・`pipe_main_same_` 4・`pipe_detection_land_` 3・`pipe_detection_oom_` 3・`pipe_detection_scope_` 3+1・`pipe_detection_single_shot_` 3+1（行 al の歯・③ を gate が 1 回撃つことを測る）・`pipe_gate_health_` 3・`pipe_gate_targets_` 2・`pipe_gate_teeth_` 2・`pipe_detection_intake_` / `pipe_detection_red_` / `pipe_detection_verdict_` / `pipe_detection_verify_` / `pipe_gate_notice_` / `pipe_record_detection_` / `pipe_record_land_` / `pipe_record_silent_` 各 1（計 40）・残す側 = 行 ak の口の歯 `pipe_landed_detection_` 6（口は不変）・③ を宣言した toy を land まで通す `pipe_follow_docs_` 1 と `pipe_follow_self_` 1（子の終わりを待つ形に直す）・`pipe_slots_threads_` 1（jobs 行は slot の stub で測り検出線に依らない＝字面の隣接だけ・不変）・`pipe_record_show_external_form` 1（gate の後の `pipe show` の行から検出線の判定行が消える＝snapshot `e2e__pipe__gate__pipe_record_show_external_form.snap` を行 an の write-set に持ち新しい形で更新）。write-set の外で検出線に触れるのは `tests/e2e/pipe.rs` の stub の定義（`verify-count.sh` の 1 行・定義のみ・触らない）と `crates/scribe2/src/pipe/cli/show.rs`（行 ak の写し `DetectionCopy` を読むだけ・消すのは gate の `Detection` と `DetectionSkip`・触らない）の 2 か所で、`pipe_confine_` の歯に検出線の字面は 0 本。
...
[[contract]]
id = "an"
title = "検出線を gate と主実測と候補の木から外し ③ を撃つことを測る既存の歯 40 本を消す（行 am の後・形 (9)(10)(13)・Detection と gate の材料の欄と DetectionSkip::SameTree を消し pipe show の snapshot を更新）"
req = ["FR8", "NFR6", "FR50"]
section = "44"
write-set = ["crates/scribe2/src/pipe/gate.rs", "crates/scribe2/src/pipe/gate/verify.rs", "crates/scribe2/src/pipe/gate/record.rs", "crates/scribe2/src/pipe/land.rs", "crates/scribe2/src/pipe/land/verify.rs", "crates/scribe2/src/pipe/train.rs", "crates/scribe2/src/pipe/dispatch.rs", "crates/scribe2-boundary/tests/e2e/pipe/gate.rs", "crates/scribe2-boundary/tests/e2e/pipe/land.rs", "crates/scribe2-boundary/tests/e2e/snapshots/e2e__pipe__gate__pipe_record_show_external_form.snap", "docs/design/gate-cost.md"]
...
growth = ["crates/scribe2/src/pipe/dispatch.rs:5"]
depends = ["am"]
done = "(9) gate・主実測・候補の木の段の列に ③ が無く（gate と主実測は ①②④）、Detection と gate の材料の欄と主実測の面の判定と DetectionSkip の SameTree と候補の木の後続ごとの検出線の段が無く、同じ木の主実測の record 1 本と木が違う周の ①②④ は変わらない (10) 追随の面の判定が再 gate を省くかだけを返し、path を読めない周は撃ち直す側のまま (13) ③ を gate と主実測と候補の木が撃つことを測る既存の歯が消え（消す 40 本と残す 10 本の数え上げは §44 の形 13・main bc88844）、③ を宣言した toy を land まで通す残りの歯は子の終わりを待ってから測って緑、pipe show の外形の snapshot は検出線の判定行の無い形で更新され、tests/e2e/pipe.rs の stub の定義と cli/show.rs は触らない 歯: pipe_detection_off_gate_ の歯が、検出線を宣言した便の gate が stub を 1 回も呼ばず verify.jsonl に kind=detection が無く land の後の主実測の record に landed の無い kind=detection が 0 本・main が crates に触れて動いた追随の再 gate も stub を呼ばない、を測る"
