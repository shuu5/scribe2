...
- 歯（置き場は既存の file・新設の test file は作らない）:
  - **行 ak**（`tests/e2e/pipe/gate.rs`・検出線の stub と着地まで通す既存の fixture の隣・接頭辞 `pipe_landed_detection_`・base では `--detection-only` を断るので全部 RED）: (a) 測った周: 宣言の検出線が stub（argv と呼ばれた回数を記録し、`mutants-diff:` の判定行を出す）で、着地した diff が `crates/` 配下に触れる `Landed` の便に口を撃つと rc 0・stdout に `detection=measured`・`verify-main.jsonl` の `landed=<着地した sha>` を持つ `kind=detection` の record が口の前より 1 本多く `line=` が stub の判定行・stub の `--base` が着地した commit の親の sha で `--teeth` が契約の filter 語・共通 verify の stub は呼ばれない・event に `detection:measured` が 1 件・`pipe show --run` に stub の判定行と `secs=` の行・台帳の見張り（§37）の記録は 0 件。(b) 面の外: 着地した diff が docs だけの便は stub が呼ばれず、`reason=outside-scope` の skip record・理由の file・`detection:skipped`、`pipe show` の行は `detection-line: absent skipped=outside-scope`。(c) 測れなかった周: stub が rc 2 の便は stub が 1 回だけ呼ばれ、record の rc が 2・`detection:unmeasured`・show の行の末尾が `unmeasured=rc-2`、段は `Landed` のまま・main の sha は動かない。(d) 遮断器: 遮断器を閉じる `--rules` の fixture（§32 の歯と同じ形）で撃つと stub は呼ばれず `unmeasured=host-closed`。(e) 的と純移動: 契約が的を持つ便は stub の argv に `--targets` と run dir の的の file が、純移動と証明された便は `--diff` と母集団の file が載る（gate と同じ付け足し）。(f) 断り: 段が `Gated` の便と、宣言に検出線の行が無い便は rc 1 で、record・写し・event がどれも増えない。外形 snapshot `pipe_external_form`（usage の land の switch）は同じ便で写す。
