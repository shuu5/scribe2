...
- 形（番号は done と 1:1）:
  ...
  - **行 al（gate の中の検出線を 1 回撃ちにする・③ はまだ gate が撃つ）**
    6. **撃ち直しを消す**: gate の行の手から検出線の rc 2 の撃ち直し（§21・`refire(`）と、record の `retried` / `retried_from` を消す。
    ...
    8. **持ち越しを消す**: `Detection::Carry` と `carry_detection(`・`patch_id(`・持ち越しの record（`carried`）と record の `patch_id` を消す。追随と候補の木の段は面の判定（pipeline.md §30）だけで撃つか省くかを決める（面に触れた周は毎回撃つ）。
