...
- 実装の決め（4 行に共通）: (i) 歯の helper は着地の後に `detection:spawned` が在れば同じ便の終えた語（measured / unmeasured / skipped）まで上限つきで待ち、無ければ待たない。行 ak の歯は口の前後の差（record +1・写しの周 +1）で測るので、行 am と行 an の後も書き換えずに緑である。land まで通り検出線を宣言する既存の歯は行 am の窓（gate も子も撃つ）で子の分だけ期待が動くので、行 am が helper で待つ形に直す（done (12b) の列挙）。(ii) 口の本体と子を起こす 1 か所は、段と event の種別を match の腕に書かない（`touches` を持つ他の doc の行の閉包を広げない）。(iii) 子の tmp worktree は着地した commit を detach で出すだけで ref を動かさない（列の次の便の CAS と交差しない）。
