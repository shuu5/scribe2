...
- **歯**:
  - in-file（行 x の write-set の `+` の file・接頭辞 `health_judge_`）: (a) 走行可能を 4 番目の欄の**分子**から読み、分母（総 process 数）に釣られない（分母だけが閾値を超える fixture を対に置く）／(b) 待ちを `procs_blocked` の行から読み、似た見出しの行に釣られない／(c) 閾値 = 倍率 × core 数で、**ちょうど**の値は「混んでいる」でない（等号を境界に置かない）／(d) 片方だけが超えた 2 形はどちらも「混んでいる」／(e) 空・数でない・行が無いの 3 形はどれも「測れない」で、「空いている」に潰れない／(f) 3 値から行動と record の字面を出す関数は「混んでいる」だけを待つ側に、「空いている」と「測れない」を撃つ側に置き、record の字面は「測れない」の周だけ載って空でも 0 でもない（「空いている」の周と字面だけが違う）。
