...
- **本節は契約表の 3 行を持つ**: 行 ak = 形 (1)〜(5) と歯 (a)〜(f)（着地後の検出の口・人が撃つ形・gate は不変）、行 al = 形 (6)〜(8) と歯 (g)〜(i)（gate の中の検出線を 1 回撃ちにする・③ はまだ gate が撃つ）、行 am = 形 (9)〜(13) と歯 (j)〜(o)（③ を gate と主実測と候補の木から外し、land が口を切り離して起こす）。順は ak → al → am（行 al は ak を、行 am は ak と al を `depends` に持つ）。3 行に割る根は gate の diff の上限（rules 行 `gate.token_cap` の 150000 byte）で、③ を gate が撃つことを測る既存の e2e の歯（`tests/e2e/pipe/gate.rs` と `tests/e2e/pipe/land.rs` の約 40 本・本文だけで約 110 KB・main 741b25d の実測）を消す差分と口を足す差分が 1 行の diff に収まらない。**「足す → 外す」の順**にするのは、外した後で足す前の窓に着地した便の検出線が 1 回も撃たれない形（C12.4 の記録の欠け）を作らないためである。ak の後で am の前は gate も検出線を撃ち、口は人が撃ったときだけ動く（便ごとに 2 回払う形は作らない）。
...
- 形（番号は done と 1:1）:
  ...
  - **行 am（③ を gate から外し、land が口を切り離して起こす）**
...
[[contract]]
id = "am"
title = "検出線を gate と主実測と候補の木から外し、land が Landed の event の後に行 ak の口を列と同じ起こし方で切り離して起こす（待たない・detection:spawned / unspawned・runner の雛形の 1 行を直す）"
req = ["FR8", "NFR6", "FR50"]
section = "44"
...
depends = ["ak", "al"]
done = "(9) gate・主実測・候補の木の段の列に ③ が無く（gate と主実測は ①②④）、Detection と gate の材料の欄と主実測の面の判定と DetectionSkip の SameTree と候補の木の後続ごとの検出線の段が無く、同じ木の主実測の record 1 本と木が違う周の ①②④ は変わらない (10) 追随の面の判定が再 gate を省くかだけを返し、path を読めない周は撃ち直す側のまま (11) finish が Landed の event の後・worktree の退避と終端の前に、宣言の写しに検出線の行が在る便だけ列と同じ起こし方の 1 本で行 ak の口を --state-dir と --repo（と受けた --rules）付きの子 process として起こして待たず、起こせた周は detection:spawned・起こせなかった周は detection:unspawned と理由の file unmeasured=unspawned と stderr の 1 行を残して rc と着地を変えず、main-red と main-unmeasured の周は起こさず、候補の木の後続も自分の finish から起こし、land の材料に --rules の path の欄が 1 つ増える (12) runner の雛形の検出線の行が着地の後に器が撃つ形になり、runner は撃たず禁じる語列は変わらない (13) ③ を gate と主実測と候補の木が撃つことを測る既存の歯が消え（消す 40 本と残す 10 本の数え上げは §44 の形 13・main bc88844）、③ を宣言した toy を land まで通す残りの歯は子の終わりを待ってから測って緑、pipe show の外形の snapshot は検出線の判定行の無い形で更新され、tests/e2e/pipe.rs の stub の定義と cli/show.rs は触らない 歯: pipe_detection_after_landing_ の歯が、gate が stub を 1 回も呼ばず verify.jsonl に kind=detection が無い・land が stub の終わる前に rc 0 で Landed を返し spawned が 1 件で終えた語が 0 件・解放の後に measured が 1 件と landed を持つ record が 1 本で主実測の record に landed の無い kind=detection が 0 本・台帳の見張り 0 件・検出線を宣言しない便は detection の detail を書かない・main-red の便は spawned を持たない・追随の再 gate も stub を呼ばない・候補の木の後続は自分の run dir に landed を持つ record を持ち --base が自分の squash commit の親、を測り、runner の雛形の snapshot と検出線の行の歯が新しい字面で緑"
