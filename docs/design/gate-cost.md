...
- 裁定: user 裁定 `2026-09-23T07:16Z`（A2・逐語は台帳 `s2-07l.172`）の要旨は「R-C6-1 の検出線 = 総 token（4 値の和）25M。断らず判定行に超過を出すだけ。値は rules 行に裁定 id つきで置く」。裁定の根拠は orchestrator の実測（同日・`RunCost` の event・50 便・中央値 1.3M / 上位 1 割 18.8M / 最大 37.9M・25M 超えは 3 便）で、上の数え直しとは母集団（便の数と日付の窓）が違うが、最大値と超過 3 便は一致する。その前の裁定 `2026-09-22T06:48Z`（同じ台帳）は「まず検出線で入れる・上限に達した便を記録して止めない」。
