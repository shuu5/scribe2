...
- 歯（置き場は既存の file・新設の test file は作らない）:
  ...
  - **行 al**（`tests/e2e/pipe/gate.rs` と `tests/e2e/pipe/land.rs`・接頭辞 `pipe_detection_single_shot_`）: (g) 共通 verify と契約 verify が緑で検出線だけ rc 2 の便の gate は PASS（lens の偽 PASS）・stub は 1 回だけ呼ばれ・record の rc は 2 で `retried` を持たない（base では撃ち直して INCONCLUSIVE ＝ RED）。(h) 共通 verify が赤で検出線が rc 2 の便は FAIL で、evidence の赤い行の数に検出線を含まない。(i) main が `crates/` に触れて動いた便の追随で、便の diff の patch-id が前周と同じでも検出線がもう 1 回撃たれ、`carried` の record と record の `patch_id` が無い（base では持ち越して撃たない ＝ RED）。§21 / §28 / pipeline.md §40 形 (b) の歯（接頭辞 `pipe_detection_retry_` / `pipe_gate_red_wins_over_detection_` / `pipe_detection_carry_` と、rc 2 を INCONCLUSIVE と測る歯）は同じ便で消す。
...
size = "M"
depends = ["ak"]
done = "(6) gate の行の手に検出線の撃ち直しが無く、record に retried と retried_from が無い (7) 判定の順に検出線の rc 2 の段が無く、検出線の rc 2 は赤にも INCONCLUSIVE にも数えず record の rc 2 のまま残り、検出線の rc 1 は赤のままで段①・箱の中の死・遮断器の順は動かない (8) Detection の Carry と持ち越しの判定と patch-id の測りと持ち越しの record と record の patch_id が無く、追随と候補の木の段は面の判定だけで撃つか省くかを決める 歯: pipe_detection_single_shot_ の歯が、検出線だけ rc 2 の便の gate が PASS で stub が 1 回だけ呼ばれ record の rc が 2・赤い共通 verify と rc 2 の検出線の便が FAIL で evidence の赤に検出線を含まない・main が crates に触れて動いた追随で patch-id が同じでも検出線がもう 1 回撃たれ持ち越しの record が無い、を測り、撃ち直し・rc 2 の INCONCLUSIVE・持ち越しを測る既存の歯は同じ便で消える"
