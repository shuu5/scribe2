...
- **§44 が上書き**（[ADR-0060](../../design-intent/decisions/ADR-0060-detection-line-runs-after-landing-not-in-the-gate.html)・契約表の行 ak / al / am の land 後・本節の本文は書き換えない）: ③ は gate でも主実測でも撃たず、便が着地した後に land が切り離して 1 回撃つ（着地後の検出・record は `verify-main.jsonl` の `landed` 付き）。検出線の rc 2 は gate の判定に入らず record に残るだけになり、木が同じ周に ③ を省く記録（`skipped=detection`）は主実測から消える。
...
- **§44 が上書き**（[ADR-0060](../../design-intent/decisions/ADR-0060-detection-line-runs-after-landing-not-in-the-gate.html)・契約表の行 ak / al / am の land 後・本節の本文は書き換えない）: 純移動の母集団の付け足しは着地後の検出が gate と同じ 1 本で行う（gate は ③ を撃たない）。着地後の検出の {base} は着地した commit の親なので、母集団は便の diff と同じ。
...
- **撃たなかった周は写さない**（設計 §30 の検出線を省いた周・その型は §44 行 ao で消えた）。撃っていない周の置き場を作ると、撃って 0 件だった周と読み分けられない（(2) の極性と同じ理由）。
...
- **§44 が上書き**（[ADR-0060](../../design-intent/decisions/ADR-0060-detection-line-runs-after-landing-not-in-the-gate.html)・契約表の行 ak / al / am の land 後・本節の本文は書き換えない）: 写しを書くのは着地後の検出になり、置き場と周の番号の関数は gate と同じ 1 本のまま。測れなかった周と撃たなかった周は同じ置き場に理由の file を置き、本 errata の「撃たなかった周は写さない」は理由の file だけを持つ置き場と読み替える。`pipe show` の読み手は変えない。
...
- **歯**（`e2e_fixture_` 接頭辞・置き場は `crates/scribe2-boundary/tests/e2e/main.rs`）: (a) 包む型を drop した後に dir が**無い**（中に file を置いた周も再帰で消える）／(b) **panic した歯**でも dir が消える（std の unwind を捕まえる口の中で作って落とし、外で不在を測る＝(1) の否定の枝・現物の形では残る・(4) の預けた包みも panic した thread の join の後に不在）／(c) guard を降ろした周は drop の後も dir が**在る**（(3) の枝・降ろす口が無ければ空虚になる pin）／(d) 作り手が 2 回続けて別の path を返す（既存の一意性が壊れていない）。
...
- **§44 が上書き**（[ADR-0060](../../design-intent/decisions/ADR-0060-detection-line-runs-after-landing-not-in-the-gate.html)・契約表の行 ak / al / am の land 後・本節の本文は書き換えない）: 同じ gate の中の撃ち直しは行 al で消える。着地後の検出の rc 2 は理由付きの record のまま残り、自動では撃ち直さない（撃ち直すのは人が同じ口を撃つ形）。
...
- **§44 が上書き**（[ADR-0060](../../design-intent/decisions/ADR-0060-detection-line-runs-after-landing-not-in-the-gate.html)・契約表の行 ak / al / am の land 後・本節の本文は書き換えない）: 木が違う周の主実測も ③ を撃たず ①②④ だけを撃つ。木が同じ周の record 1 本は変えない。
...
- **§44 が上書き**（[ADR-0060](../../design-intent/decisions/ADR-0060-detection-line-runs-after-landing-not-in-the-gate.html)・契約表の行 ak / al / am の land 後・本節の本文は書き換えない）: 検出線の rc 2 の段は行 al で判定の順から消える（赤の数え方と、段①・箱の中の死・遮断器の順は変えない）。
...
- **現物**（本行の base・verified）:
  ...
  - PATH を差し替える口は `run_pipe_with_path(` の 17 か所だけである（`crates/scribe2-boundary/tests/e2e/pipe/gate.rs` 10・`crates/scribe2-boundary/tests/e2e/pipe/stop.rs` 4・`crates/scribe2-boundary/tests/e2e/pipe/spawn.rs` 2・`crates/scribe2-boundary/tests/e2e/pipe/launch_failure.rs` 1）。
...
- **約束**（番号は done と歯に 1:1 で対応する）:
  1. **道具箱は 1 本**: 歯が toy repo で実 binary を撃つときの PATH の組み立ては `crates/scribe2-boundary/tests/e2e/main.rs` の 1 関数（`make_tmp_dir(` の隣・全 module から見える可視性）に寄り、偽 systemd-run と偽 systemctl を置いた dir を先頭に積んだ PATH の値を返す。host の PATH は後ろに残る（git / sh / cargo の解決は不変）。
...
- **歯**:
  ...
  - e2e（`crates/scribe2-boundary/tests/e2e/pipe/gate.rs`・接頭辞 `pipe_slots_threads_`・既存の受付の歯と同じ toy の形）: 3 つの穴を持つ行の置換後の `cmd` に実効 jobs と実効 thread が**両方**載り、**待ちの上限を超えた周の `cmd` は jobs も thread も 1** である（`slot=degraded` の record と対で 1 本）。
...
- **歯の fixture**: e2e の rules fixture（`crates/scribe2-boundary/tests/e2e/pipe.rs` の `SlotFixture`）は倍率の既定を十分大きな値に置く＝並列の歯で混んだ host でも既存の歯の遮断器は閉じない。倍率を振るのは接頭辞 `pipe_gate_health_` の歯だけである。
...
- **歯**:
  ...
  - 行 aa（e2e・`crates/scribe2-boundary/tests/e2e/pipe/gate.rs`・接頭辞 `pipe_gate_teeth_`・toy repo の宣言に `{teeth}` を持つ検出線を置く）: 撃たれた行の record に契約の verify 行の語が `,` で結ばれて載り、`{teeth}` の字面が残らない。
...
- **§44 が上書き**（[ADR-0060](../../design-intent/decisions/ADR-0060-detection-line-runs-after-landing-not-in-the-gate.html)・契約表の行 ak / al / am の land 後・本節の本文は書き換えない）: `{teeth}` を置くのは gate と主実測でなく着地後の検出で、導出は同じ 1 本（契約の写しの verify 行から）。
...
- **形**（値の変更だけ・rules 行の形は不変）: `rules/manifest.toml` の行 `gate.slot_wait_s` の `value` を 900 → 5400、`ruling` を裁定の id、`ruled_at` を裁定日にする。`kind` / `enabled` / 他の 5 行は不変。歯の pin（`crates/scribe2-boundary/tests/e2e/rules.rs` の `rules_embedded_manifest_declares_the_gate_cost_rows`・6 行の表）の該当行を新しい値と裁定 id と裁定日にする。§3 の表と rules-manifest.md は値を持たないので不変。
...
- **何が起きているか**（本行の base・main 4f70b12・verified）:
  - 道具箱を組む 1 関数（`crates/scribe2-boundary/tests/e2e/main.rs` の `toolbox_path`）は、呼ばれるたびに偽 systemd-run（`write_systemd_run_stub`）と偽 systemctl（`write_systemctl_stub`）を**同じ置き場の同じ名へ書き直す**。書き方は std の fs の write で、既に在る file を**その場で切り詰める**＝走っている process が握っている本体（inode）に触る。
  - その口は 1 便 1 回ではない。`crates/scribe2-boundary/tests/e2e/pipe.rs` の `pipe_cmd` が --state-dir を持つ起動の**たびに**道具箱を組み直す＝同じ歯の中で binary を 2 度撃つ周は、1 度目が起こした子 process が偽 systemd-run を exec している最中に 2 度目が同じ本体へ書く。落ちた歯はまさにその形（`turn` を 4 度撃ち、1 度目が 2 便を背景で起こす）である。
...
- **形**（番号は done と歯に 1:1 で対応する）:
  1. **道具箱に見張りを 1 本置く**: 道具箱を組む 1 関数（§30 約束 1・`crates/scribe2-boundary/tests/e2e/main.rs` の `toolbox_path`。偽 systemd-run と偽 systemctl を bin dir に書いて PATH の先頭に足す）が、その 2 本に並べて**台帳 client の既定名の偽物**（既定名は `crates/scribe2/src/seat/ledger.rs` の `DEFAULT_BD`）を置く。呼ばれた argv を 1 起動 1 file で記録 dir へ写してから、**台帳を解けない host と同じ形で断る**（rc は非 0・標準出力は空）。
...
- **歯**（接頭辞 `e2e_ledger_tripwire_`・置き場は `crates/scribe2-boundary/tests/e2e/pipe.rs` と列の歯の file `crates/scribe2-boundary/tests/e2e/pipe/dispatch.rs`。行の 2 本目の verify が名指す既存の歯 `pipe_terminal_dispatch_manual_turn_starts_the_runs_it_can` も同じ `pipe/dispatch.rs` に在る）:
...
- 歯（接頭辞 `pipe_scope_reap_`・`crates/` 全体の fn 名の substring として base に 0 件。in-file は `crates/scribe2/src/pipe/confine.rs` の `mod tests`・e2e は偽 `systemctl` の呼出を記録する fixture が既に在る `crates/scribe2-boundary/tests/e2e/pipe/gate.rs`）:
...
- 形（番号は done と 1:1）:
  1. **段ごとの対**（(A) の解）: hunk の本文を「`-` の連続 k 行の直後に `+` の連続 k 行」の**段**に切り、段と段の間と前後に context が在る形（段が 1 つの周は §41 形 1 の 1 塊と同じ）を許す。段ごとに `-` の各行が置換で 1 字以上変わり、置換後の列が `+` の列と順序も本数も同じなら hunk 全体を 1 行の印に畳む（印の件数は全段の `-` / `+` の合計・本数の違いは列の相等の判定に含まれ、別の条件を持たない）。`-` の連続の直後が context（`+` が続かない）か、段の本数が違えば逐語のまま＝§41 の歯 (f)(h) の形は今までどおり逐語。
  2. **移動で空になった dir の対**（(B) の解）: `measure` が HEAD の tracked path の列（`git ls-tree -r --name-only HEAD`）を**読めた周だけ**（`Option`・読めない周と列が空の周は無し）pure な 1 本に渡す。1 本は列が無い周に **dir の対の導出を丸ごと行わない**（空の列を渡すと (i) が空虚に真になり、共通 prefix の広い対〔却下 2 の形〕が生まれて移動していない file の書き換えを隠す＝便 174218Z の gate FAIL・fail-open）。1 本は rename の対 (a, b) ごとに末尾の共通の path component を 1 つずつ剥がした prefix の対 (pa, pb) を**長い pa から順に**見て、(i) HEAD の列に `pa/` で始まる path が 1 つも無く、(ii) `pa/` で始まる旧 path を持つ rename の対が全部 `pb/` + 同じ相対 path へ行く、の両方を満たす間だけ dir の対として足す（どちらかが破れたら、それより短い prefix は見ない）。dir の対は file の対と同じ列に入り、長い旧 path から順に置換する（本便の diff では 6 対: tests・tests/e2e・tests/e2e/pipe・tests/e2e/seat・tests/e2e/snapshots・src/snapshots）。
  3. 通知と判定行の字面は §41 のまま（`elided=<hunk 数>/<行数>`・`bytes=` は畳んだ本文・`diff_bytes` は生 diff）。
  4. 歯（e2e・`crates/scribe2-boundary/tests/e2e/pipe/gate.rs`・接頭辞 `pipe_gate_elide_` のまま・§41 の 8 本は 1 字も変えず緑のまま）: (i) 2 段の hunk（`-A` / `+A'` / context / `-B` / `+B'`・どちらの段も置換だけ）→ 畳む・通知に `elided=1/4`。(j) 段の本数が違う（`-A` / `-B` / `+A'`）→ 逐語。(k) rename が tests 配下の file 1 本から boundary 側の同じ相対 path への 1 本で、HEAD に `tests/` 配下の path が無く、docs の行が `tests/` の dir だけを名指す置換 → 畳む。(l) (k) と同じで HEAD に `tests/` 配下の path が 1 つ残る → 逐語（dir の対を足さない）。(m) (k) と同じで `tests/` 配下のもう 1 本の rename が別の dir へ行く → 逐語（一貫しない dir は対にしない）。(n)（unit・`crates/scribe2/src/pipe/gate/lens.rs` の歯の区間・接頭辞 `gate_elide_dir_pairs_`）: rename の対が在って HEAD の列が無い周は dir の対が 0 で、同じ rename の対に配下の path を持たない列を渡した周は対が在る（対照）。変異の A/B（判定の順は 段の切り分け → 段ごとの一致 → 効き、dir の対の導出は (i) 空 → (ii) 一貫・条件 1 つに歯 1 本）: 段の切り分けを 1 塊に戻すと (i) が落ちる・dir の対の導出を外すと (k) が落ちる・(i) 空の条件を外すと (l) が落ちる・(ii) 一貫の条件を外すと (m) が落ちる・列が無い周の分岐を外して空の列として扱うと (n) が落ちる。(j) は段の本数の違いが列の相等（§41 の一致の判定・歯 (b) が測る）で逐語に落ちることを固定する回帰の歯で、単独の変異を持たない。§41 の (a)〜(h) も本便の変異のどれでも落ちない回帰の歯。
...
## 43. 便ごとの token 消費の検出線 — rules 行 R-C6-1 の値 25M を `pipe show` の判定行 1 行に読む（契約表の行 aj・`s2-07l.172`・断らない）

- 何が起きているか（実測 2026-09-24・main b82be74・verified）: 憲法 C6.2 は便ごとの token 上限（§3 の R-C6-1）を名指すが、`rules/manifest.toml` に `R-C6` で始まる行は 0 件で、`cargo xtask check` の rules-parity は `doc-only=1 ids=R-C6-1` を出す（[rules-manifest.md](./rules-manifest.md) §4 の「現物の期待値」）。消費は §26 行 r の着地で `fleet/events.jsonl` の `RunCost` の event に記される（書き手は `record_cost` の 1 本・呼び手は spawn / gate / review の 3 か所・6 値のどれかが欠けた周は書かない）。読み手は `pipe show --run` の `cost_lines`（便の event を 1 行ずつ写し、先頭に母集団 `cost: events=<n>`）と `pipe report` の `Spent`（全便の out と cache_read の和）の 2 つで、**便ごとの 4 値の和と閾値を比べる口はどこにも無い**。置き場を同じ時点で数え直すと、`RunCost` は 119 件・65 便で、runner の event を持つ 32 便の総 token（in + out + cache_read + cache_create の和）は中央値 3.98M・上位 1 割 24.0M・最大 37.9M、25M 以上は 3 便（3 便とも同じ bead の便）。審査の event だけを持つ 33 便は受付で止まった便で、全 65 便の中央値は 0.09M に落ちる。
- 裁定: user 裁定 `2026-09-23T07:16Z`（A2・逐語は台帳 `s2-07l.172`）の要旨はR-C6-1 の検出線を総 token（4 値の和）25M とし、便を断らず判定行に超過を出すだけにして、値を rules 行に裁定 id つきで置くこと。裁定の根拠は orchestrator の実測（同日・`RunCost` の event・50 便・中央値 1.3M / 上位 1 割 18.8M / 最大 37.9M・25M 超えは 3 便）で、上の数え直しとは母集団（便の数と日付の窓）が違うが、最大値と超過 3 便は一致する。その前の裁定 `2026-09-22T06:48Z`（同じ台帳）の要旨は、検出線を先に入れ、上限に達した便を記録して止めないこと。
- やさしく言うと: 1 便が使った token の合計が 2500 万に届いたら、`pipe show` にその便の「超えた」の 1 行が出る。便は止めない。止めるかどうかは、この行が何便に出るかを見てから user が決める。
- 形（番号は done と 1:1）:
  1. **rules 行**: `rules/manifest.toml` に行 `R-C6-1` を 1 本足す（kind は Int の形の新しい variant 1 つ・名は実装が決める・`value = 25000000`・`enabled = true`・`ruling = "user 2026-09-23T07:16Z"`・`ruled_at = "2026-09-23"`・行の上の注記は逐語を書かず本 § を指す）。variant は `crates/scribe2/src/rules/mod.rs` の `RuleKind` に `GateTokenCap` の直後（同じ token の上限の族）で足し、`ALL` と字面と値の形の腕を同じ位置に揃える（末尾に足すと host の見張りの末尾 3 種を pin する既存の歯が動く）。`enabled = true` は**検出線の読み手を効かせる**意味で、停止の読み手は器に無い（形 4）。[rules-manifest.md](./rules-manifest.md) §4.1 の表に行を 1 本足し、同 § の「置かない行」から R-C6-1 を外し、「現物の期待値」を `doc-only=0 ids=-` に写す。埋め込み manifest の行数と kind 数を pin する既存の歯 2 か所（`tests/e2e/rules.rs`）と外形 snapshot（`rules validate` の `rows=` / `kinds=` の 2 行）は land 時の現物に +1 で写す。
  2. **判定の 1 本**: `crates/scribe2/src/pipe/cli/show.rs` に pure な 1 本を足す。入力は置き場の event 列・便 id・閾値で、便の `RunCost` の event の 4 値（in / out / cache_read / cache_create・turns と wall_ms は足さない）を event をまたいで飽和加算し、和が閾値**以上**なら 1 行 `cost-ceiling: over total=<和> limit=<閾値> events=<その便の消費の event 数>` を返す。和が閾値未満の便と消費の event が 0 件の便は行を返さない。「以上」は 09-22 の裁定の「上限に達した便」に合わせる。
  3. **読みの口**: `pipe show --run` が `cost_lines` の後に形 2 の行を置く。閾値は `manifest_of`（`--rules` か埋め込み）と `int_row` で行 `R-C6-1` から読み、行の id は `show.rs` の const 1 つが持つ（rules-wired の読み手）。消費の event が 1 件以上在るのに閾値を読めない周（manifest が読めない・行が無い・不発効・整数でない）は `cost-ceiling: unmeasured events=<n>` の 1 行を出す（「超えていない」に倒さない・C10）。1 行目・検出線の行・消費の行の字面と rc は変えず、`pipe show` は event を 1 件も書かない。
  4. **断らない**: 受付・spawn・gate・land・driver は行 `R-C6-1` を読まない＝和が閾値を超えた便も今と同じ段を進み Landed に着く。停止（C6.2 の Completion の 1 値）は本行の外で、値と形は判定行が出た便の数を見てから user が A2 で裁定する。
- 現物の閉包（verified・main b82be74）: `RuleKind` の腕を持つ file は `rules/mod.rs` だけで（`as_str` / 値の形 / `ALL`・validate の特殊腕は List の kind だけ）、Int の新しい kind は loader の既定の検査で通る。`pipe show` は `--rules` を既に受ける（`ALLOWED_SHOW` の置き場の 3 flag）。`manifest_of` と `int_row` は `pipe/cli/args.rs` に在り `show.rs` から親経由で引ける＝`args.rs` と `cli.rs` は触らない。既存の消費の歯は行の先頭一致と部分一致の 2 形で `cost:` の字面を選ぶので、`cost-ceiling:` の行はどちらにも当たらない（`cost-ceiling:` は `cost:` を部分文字列に持たない）。偽 lens / 偽 runner の usage は 1 桁〜2 桁の token で閾値に届かない＝既存の描画と外形 snapshot（`pipe_record_show_external_form`）は動かない。
- 触らない: `RunCost` の event の形と書き手（§26 行 r）・`pipe report` の 2 行（`Spent`）・`Budget` と唯一の起動口（C6 前段・`pipe/mod.rs` の doc の「R-C6-1 が未定」は停止の便で直す）・行 `gate.token_cap`（diff の byte の上限で消費ではない）・席の cycle / inject の面（ADR-0045 §2 (2) で消えた・本行は便の消費だけ）・憲法 §3 の R-C6-1 の初期値のセル「未定」（値の正本は manifest・セルの書き換えは user の `/folio-architect` の手番）。
- 却下:
  - **`enabled = false` で置く**（09-22 の裁定の字面）: `int_row` は不発効の行を読まないので判定行が 1 本も出ず、検出線にならない。09-23 の裁定が値を決めて「判定行に超過を出す」と言ったので、発効させて停止の読み手を持たないことで「止めない」を満たす。
  - **`RunCost` を書く周（`record_cost`）で和を数え直し、超えた周に event か stderr を足す**: 書くたびに store 全体を読み直し、超過の事実を消費の event から導いた派生値として 2 つ目の記録に持つ（C6.3 の store 1 つ・C10）。stderr は runner / gate の log に埋もれて読み手が無い。
  - **gate の verify record に載せる**: gate の後にも review と runner の焼き直しが消費を足すので、gate の周の和は便の和でない。verify record は機械検証の段の記録で、消費の記録ではない。
  - **land の終端（`Landed` の detail）に載せる**: land に着かない便（FAIL・止めた便）は判定されず、detail の字面を pin する歯が広く動く。
  - **`pipe report` に全便の超過の件数を足す**: 便ごとの値が見えず、同じ和を 2 か所で数える口が要る。件数は `pipe show` の行を便ごとに数えれば出る。要るなら判定の 1 本を再利用する別の便で足す。
  - **閾値を「超え」（`>`）にする**: 09-22 の裁定は「上限に達した便」。境界の 1 token の差で運用は変わらないので、裁定の字面に合わせる。
- 歯（置き場は既存の file・新設の test file は作らない）:
  - (a) 値の両側と断らない（`tests/e2e/pipe/spawn.rs`・接頭辞 `run_cost_ceiling_`）: 置き場を 2 つ作り、どちらも偽 runner の要約行（spawn）と偽 lens の判定 object（gate）で消費の event を 2 件書いて land まで撃つ。4 値はどれも 0 でない数で、一方は 2 件の和が `25000000` ちょうど、もう一方は `24999999`。和がちょうどの便は land が rc 0・段が `Landed` で、`pipe show --run`（`--rules` 無し＝埋め込み manifest）に `cost-ceiling: over total=25000000 limit=25000000 events=2` の 1 行がちょうど 1 本在る。`24999999` の便も `Landed` で、`cost-ceiling:` で始まる行は 0 本。消費の行と母集団の行（`cost:` で始まる行）は両方の便で従来の形のまま 3 行。4 値のどれか 1 つを和から落とす実装、最後の event だけを数える実装、`>` で比べる実装は、ちょうどの便で行が消えて落ちる。
  - (b) 読めない閾値（同じ file・同じ接頭辞）: 消費の event を 2 件持つ便に、行 `R-C6-1` を持たない `--rules` の fixture（`ceiling_rules`）と、行を `enabled = false` で持つ fixture の 2 つで `pipe show --run` を撃つと、どちらも `cost-ceiling: unmeasured events=2` が 1 行在り、rc は 0 で 1 行目は段の行のまま。同じ便を `--rules` 無しで撃つ対照は (a) の over の行を出す。消費の event が 0 件の便は、読めない fixture でも `cost-ceiling:` の行が 0 本（従来の描画のまま）。
  - (c) 行の実在（`tests/e2e/rules.rs`・接頭辞 `rules_embedded_manifest_declares_run_token_ceiling_`）: 埋め込み manifest の行 `R-C6-1` が値 `25000000`・値の形 Int・発効・裁定 id `user 2026-09-23T07:16Z`・裁定日 `2026-09-23` を持ち、`int_row` で 25000000 が取れ、kind が `ALL` で `GateTokenCap` の直後に在り、字面から引ける。
  - 既存の歯の写し（同じ便で更新）: 行数と kind 数の pin 2 か所（`tests/e2e/rules.rs`）と外形 snapshot `rules_external_form`。base では (a) の over の行・(b) の unmeasured の行・(c) の行が無いので RED。

## 44. 検出線は gate で撃たず、便が着地した後に land が切り離して 1 回撃つ（契約表の行 ak / al / am・[ADR-0060](../../design-intent/decisions/ADR-0060-detection-line-runs-after-landing-not-in-the-gate.html)・`s2-07l.465`）

- **本節は契約表の 5 行を持つ**: 行 ak = 形 (1)〜(5) と歯 (a)〜(f)（着地後の検出の口・人が撃つ形・gate は不変）、行 al = 形 (6)〜(8) と歯 (g)〜(i)（gate の中の検出線を 1 回撃ちにする・③ はまだ gate が撃つ）、行 am = 形 (11)(12) と歯 (k)〜(o)（land が口を切り離して起こす・③ はまだ gate も撃つ＝窓の便は二重に測られる）、行 an = 形 (9) と (13) の gate 側と歯 (j)(n)（gate の段の列から ③ を外す・型と欄は残す・既存の歯 23 本を消し 4 本を直す）、行 ao = 形 (9) と (13) の残りと形 (10) と歯 (p)（主実測と候補の木から ③ を外し `Detection` と欄と DetectionSkip の SameTree を消す・既存の歯 8 本を消し 2 本を直す）。順は ak → al → am → an → ao（行 al は ak を、行 am は ak と al を、行 an は am を、行 ao は an を `depends` に持つ）。行 am と行 an に割る根も同じ cap: 便 s2-07l.589-20260924T042320Z（形 (9)〜(13) を 1 便）の diff は 212339 byte で cap 超え（e2e の `pipe/gate.rs` だけで 97372 byte・消す歯 49 本）。3 行に割る根は gate の diff の上限（rules 行 `gate.token_cap` の 150000 byte）で、③ を gate が撃つことを測る既存の e2e の歯（`tests/e2e/pipe/gate.rs` と `tests/e2e/pipe/land.rs` の約 40 本・本文だけで約 110 KB・main 741b25d の実測）を消す差分と口を足す差分が 1 行の diff に収まらない。**「足す → 外す」の順**にするのは、外した後で足す前の窓に着地した便の検出線が 1 回も撃たれない形（C12.4 の記録の欠け）を作らないためである。ak の後で am の前は gate も検出線を撃ち、口は人が撃ったときだけ動く（便ごとに 2 回払う形は作らない）。
- 何が起きているか（ADR-0060 の文脈・run dir の verify record・2026-09-21 以後に起こした便の 164 周）: gate 1 周の verify 行の秒の和の中央値 202 秒のうち、検出線が中央値 97.5 秒（周ごとの割合の中央値 53%・上位 1 割は 517 秒以上）を占める。検出線は止めない線（C12.4・R-C12-1 は deny でない）なのに臨界路に座り、rc 2 を INCONCLUSIVE に倒す判定（§5・§28）・同じ gate の中の撃ち直し（§21）・持ち越し（[pipeline.md](./pipeline.md) §40 形 (b)）・追随と主実測の面の判定（pipeline.md §30）が検出線のためだけに在る。
- 裁定: user 裁定 `2026-09-23T07:16Z`（A2・逐語は台帳 `s2-07l.465`）の要旨は、変異検査の検出線を gate の臨界路から外して gate は止める段だけで判定を出し、検出線は着地の後に撃って結果を後から記録と event に載せること。
- 現物（verified・main 741b25d）: gate は ③ を ①②④ と同じ手（`run_checks_admitted(`）で撃ち、的（§16）と純移動の母集団（§14）の付け足しと写しの周の番号は gate の材料を引数に取る 4 本（`aimed_lines(` / `population_lines(` / `keep_detection(` / `round_of(`）が持つ。受付札を取るのは gate だけで、主実測（`land/verify.rs`）は ③ を面の判定（行 ao で消えた main_detection）で省くか受付を通さずに撃つ。着地した commit を tmp worktree に出して宣言の写しの行を撃つ形・箱・遮断器・{teeth} の導出・写しの置き場は在る。足りないのは 5 つ（口・切り離し・gate の材料からの剥がし・③ だけを撃つ入口・写しの置き方〔理由の file〕）。
- 形（番号は done と 1:1）:
  - **行 ak（着地後の検出の口）**
    1. **口**: `pipe land` に値の無い switch `--detection-only` を足す（`--terminal-only` と同じ形の口）。段が `Landed` の便だけを通し（他の段は何も書かずに rc 1）、着地した sha は `landed_sha(` で記録から読み（読めない周は断る）、宣言の写しに検出線の行が無い便は何も書かずに rc 1 で断る。stdout は `run=<id> detection=<語>` の 1 行で rc 0（測れなかった周も rc 0＝検出線は止めない）、record・写し・event を書けない周だけ rc 2。本体は本行の write-set の + の file に置き、`pipe/land.rs` には mod 宣言だけを足す。
    2. **撃つ**: 着地した commit を主実測と同じ置き場の別名（便 id に検出線の接尾辞を付けた名）へ `git worktree add --detach` で出し、{base} = 着地した commit の親の sha・{teeth} = 契約の写しの verify 行から（§34 と同じ導出）・行 = 凍結した宣言の写しの `detection-verify`。上の 4 本は gate の材料でなく「置き場と便 id」の対を取る形に直して gate と口の両方から呼ぶ（的と純移動の付け足しと周の番号は 1 実装・C2）。③ の行だけを撃つ入口を `gate/verify.rs` に 1 本足し、行ごとの受付（gate と同じ材料で組む受付札・`gate.slot_wait_s` を過ぎた周は並列度 1 に縮退・FR46）と箱と遮断器は `run_checks_admitted(` と同じ行の手を通す（①②④ は撃たない・§21 の撃ち直しは持たない）。撃った後に worktree を `git worktree remove --force` で畳む。
    3. **面**: 着地した commit と親の path（`git diff-tree -r --name-only -z`）が `DETECTION_SCOPE` に 1 つも触れない周は撃たず、skip record（`reason=outside-scope`）を残す。path を読めない周は撃つ（読めないを「触れない」に読み替えない・fail-closed）。
    4. **記す**: 撃った便自身の run dir の `verify-main.jsonl` に record を追記する（無ければ作る・`n` は既存の非空の行の次・schema 1 のまま・`kind=detection`・`tree` = 着地した木〔親か木を読めない周は主実測の `unknown` の語〕）。本数は、撃った周は写しの検出線の行ごとに 1 本、撃たなかった周と撃てなかった周（面の外・遮断器が閉じた・worktree か親を出せない）は理由を持つ 1 本＝**どの周も 1 本以上**（着地した便に `landed` 付きの record が 1 本も無い周を作らない・C12.4 の記録の欠け）。任意 field **`landed=<着地した sha>`** が、主実測の record と本行より前の主実測の `kind=detection`（`landed` を持たない）から着地後の検出の record を分ける。写しは §15 の置き場（周の番号は gate と同じ 1 本）に置き、測れなかった周（rc が 0 と 1 以外・遮断器が閉じた・worktree か親を出せない）と撃たなかった周は同じ置き場に**理由の file** を 1 つ置く（中身は `unmeasured=<rc-<rc>|host-closed|unprepared>` か `skipped=outside-scope` の 1 語）。`RunDone stage=Landed` の detail に `detection:measured` / `detection:unmeasured` / `detection:skipped` のどれか 1 語を 1 件記す（着地の sha も `terminal:` の前置きも持たない＝`landed_sha(` と終端の読み手は読み飛ばす）。台帳 client は呼ばない（C15・FR50）。
    5. **読む**: `pipe show --run` の読み手（写しの周ごとの 1 行）は変えない。写しを読む 1 本が、理由の file の在る周だけ行の末尾（判定行の無い周は不在の 1 行 `detection-line: absent` の後）に理由の 1 語を置く。理由の file の無い周の字面は 1 字も変わらない（出所は写し 1 本のまま・§15 (3)）。
  - **行 al（gate の中の検出線を 1 回撃ちにする・③ はまだ gate が撃つ）**
    6. **撃ち直しを消す**: gate の行の手から検出線の rc 2 の撃ち直し（§21 の撃ち直しの関数）と、record の `retried` / `retried_from` を消す。
    7. **rc 2 を判定から外す**: 判定の順（`machine_order(`）から検出線の rc 2 の段（§5 の rc 2 → INCONCLUSIVE・§28）を消し、検出線の rc 2 は赤にも INCONCLUSIVE にも数えず record の rc 2 のまま残す（測れなかったは record に残り、gate の判定は検出線を名乗らない・ADR-0060）。検出線の rc 1 は従来どおり赤で、段①が読めない・箱の中の死・遮断器の順は動かない。
    8. **持ち越しを消す**: `Detection` の持ち越しの variant と、持ち越しの判定の関数・patch-id を測る関数・持ち越しの record（`carried`）と record の `patch_id` を消す。追随と候補の木の段は面の判定（pipeline.md §30）だけで撃つか省くかを決める（面に触れた周は毎回撃つ）。
  - **行 am（land が口を切り離して起こす・③ はまだ gate も撃つ）**
    11. **起こす**: `finish(` が `Landed` の event を書いた後・worktree の退避と終端（`terminal(`）の前に、宣言の写しに検出線の行が在る便だけ、列が便を起こすのと同じ 1 本（`spawn_self(`・起こす側と起こし直す側の 1 実装・C2）で行 ak の口（`pipe land --run <id> --detection-only --state-dir <S> --repo <R>`・land が `--rules` を受けた周はそれも渡す）を子 process として起こし、終わりを待たない（新しい process group・stdin と stdout は null・stderr は列と同じ起動の log）。起こせた周は `detection:spawned`、起こせなかった周は `detection:unspawned` を 1 件記し、§15 の置き場に理由の file（`unmeasured=unspawned`）を置いて stderr に理由の 1 行を足す（rc と着地は変えない。理由の語と理由の file の書き手は行 ak の `land/detection.rs` に在るので、`unspawned` は同 file の閉じた語の列に足し、書き手を pub(super) に上げて `finish(` から呼ぶ）。main-red と main-unmeasured の周は `finish(` に届かないので起こさない。land の材料に `--rules` の path の欄を 1 つ足す（構築点は `cli/step.rs` の 2 か所・`train.rs`・`queue.rs` の歯）。候補の木で着地した後続も便ごとの `finish(` から起こす＝record は後続自身の `verify-main.jsonl`（新しく作る）。重い子（検出線の行）は子の中で箱に入り、並列度は受付札から来る（NFR6・AC20）。
    12. **雛形**: runner の雛形（`crates/scribe2/src/headless/runner.txt`）の「検出線は gate が撃つ」の 1 行を「着地の後に器が撃つ」に直す（runner は撃たない・禁じる語列は変えない）。
  - **行 an（③ を gate と主実測と候補の木から外す・行 am の後）**
    9. **外す**: gate・主実測・候補の木の段の列から ③ を外す（gate と主実測の段は ①②④・③ は行 ak の入口だけが撃つ）。`Detection` と gate の材料の欄、主実測の面の判定と DetectionSkip の SameTree、候補の木の後続ごとの検出線の段を消す。同じ木の主実測の record 1 本（§27・ADR-0043）と、木が違う周の ①②④ は変えない。**site の数え上げ（実測・main c135769・comment を除く code 行・`DetectionCopy` は除く・`Detection` / `DetectionSkip` / `SameTree` / 面の判定と再 gate の判定の 4 関数 / `Skipped` の 3 構築の字面）**: `pipe/gate.rs` 7（型 2 つの定義と `Gate` の欄）・`pipe/gate/record.rs` 20（`Detection` の match・`Skipped` の 3 構築・段の名）・`pipe/gate/verify.rs` 5（段の列 `CHECKS` と撃つ側の段＝列から ③ を外す・`Check` の変種は行 ak の口が撃つ段の名なので残る）・`pipe/land.rs` 14（再 gate の `Gate` の構築点・追随の面の判定と再 gate の判定と持ち越し・歯の区間 2）・`pipe/land/verify.rs` 8（主実測の面の判定と skip）・`pipe/train.rs` 5（候補の木の後続ごとの match と `Checks`）・`pipe/cli/step.rs` 2（gate の `Gate` の構築点＝欄を消すと動く）＝計 61 site / 7 file で、全部が行 an の write-set。便の割り当て: 行 an は gate の段の列から ③ を外すだけ（`pipe/gate/verify.rs` の段の列と `pipe/gate/record.rs` の gate 側＝Detection の Run の周も撃たず写しも書かない・型と欄と他の site は残る）。gate の段の列は **`&[Check]` 型の const で書かない**（xtask の enum-slices は閉じた enum の const slice に全 variant を求め、意図した部分集合の const を断る＝rules-manifest.md・.603 の 3 回目の Gated FAIL）: `CHECKS` は全変種の列のまま残し、gate の撃つ側が `CHECKS` から ③ を除いた列を**関数**で組む（順序は宣言順のまま・land の主実測と候補の木は `CHECKS` のまま）、行 ao が残り（型 2 つと `Gate` の欄・`pipe/land.rs`・`pipe/land/verify.rs`・`pipe/train.rs`・`pipe/cli/step.rs`・`pipe/gate/record.rs` の `Skipped` の 3 構築）を消す。site 0 の file: `pipe/dispatch.rs`（write-set に持たない）・`pipe/land/detection.rs` と `pipe/land/finish.rs`（行 ak の口・`Checks` の欄 `detection` は口が自分の線で埋めるので欄は残す・触らない）・`pipe/cli/show.rs`（`DetectionCopy` だけ・触らない）。
    10. **面の判定を 1 役にする**: 追随の面の判定（当時の follow_detection）は、再 gate を省いて前周の PASS を引き継ぐか（pipeline.md §33）だけを返す形に直す（path を読めない周は撃ち直す側のまま）。面の判定は pipeline.md §33 の省略と行 ak の (3) の 2 か所に残る。
    13. **歯の置き換え**: ③ を gate と主実測と候補の木が撃つことを測る既存の歯（写し・{teeth}・的・純移動の母集団・段の順・面の判定・③ の箱の死・段の秒）を消す。同じ性質は行 ak の歯 (a)〜(f) が口の上で測る。③ を宣言した toy を land まで通す残りの歯（§27 の同じ木の歯など）は、行 ak の helper（下の「実装の決め」(i)）で子の終わりを待ってから測る。**数え上げ（実測・main df06c83・数える規則 = 本体〔comment を除く〕に detection / verify-count の字面を持つか、それを持つ fixture の helper を呼ぶ #[test] fn）**: `tests/e2e/pipe/gate.rs` は fn 281 本中 歯 52・helper 34、`tests/e2e/pipe/land.rs` は fn 224 本中 歯 10・helper 5 で、全部が行 an の write-set の中。歯 62 本の内訳（接頭辞・本数・land.rs の分は +1 で示す）: 消す側 31 = ③ を gate・主実測・候補の木が撃つことを名指して測る歯 = `pipe_gate_detection_` 8（写し 4・純移動の母集団 4）・`pipe_detection_scope_` 3+1・`pipe_detection_single_shot_` 3+1（行 al の歯）・`pipe_detection_land_` 3・`pipe_detection_oom_` 3・`pipe_gate_targets_` 2・`pipe_gate_teeth_` 2・`pipe_detection_red_` / `pipe_detection_verdict_` / `pipe_detection_verify_` / `pipe_record_detection_` / `pipe_record_land_` 各 1（同じ性質は行 ak の歯 (a)〜(f) が口の上で測る）・直す側 6 = ③ の期待だけを外す歯 = `pipe_gate_health_` 3（撃った本数の probe を検出線の stub から共通 verify の stub に替える・§32 の遮断器の歯は消さない）・`pipe_main_same_tree_` の木が違う周と tree の無い周の 2（段の列を ①②④ にし ③ の不在を測る）・`gate_secs_only_fired_steps_carry_the_wall_clock` 1（母集団の record を 3 本にする）・残す側 25 = 行 ak の口の歯 `pipe_landed_detection_` 7（口は不変）・行 am の子の歯 `pipe_detection_after_landing_` 4+1・`pipe_main_same_tree_writes_one_record_and_fires_nothing` 1（done (9) の「同じ木の主実測の record 1 本」の不変を測る歯＝残す）・`pipe_detection_intake_` 1（受付の断りは ③ に依らない）・`pipe_follow_docs_` 1（子の終わりを待つ形は行 am で済み）・fixture 経由で検出線を宣言するが ③ を名指す assert を持たない 10（`pipe_land_main_` 3・`pipe_train_` 3・`pipe_record_common_` / `pipe_record_red_` / `pipe_record_silent_` 各 1・`pipe_record_show_external_form` 1＝gate の後の `pipe show` の行から検出線の判定行が消える＝snapshot `e2e__pipe__gate__pipe_record_show_external_form.snap` を行 an の write-set に持ち新しい形で更新。残りは緑のまま・動いた歯は同じ便で直す）。便の割り当て（.603 の 2 回目の diff 178302 byte が cap 超え＝2 便に割った）: 消す 31 のうち行 an = gate 側 23（`pipe_gate_detection_` 8・`pipe_detection_oom_` 3・`pipe_detection_single_shot_` 3+1・`pipe_gate_targets_` 2・`pipe_gate_teeth_` 2・`pipe_detection_red_` / `pipe_detection_verdict_` / `pipe_detection_verify_` / `pipe_record_detection_` 各 1）、行 ao = 主実測と候補の木の側 8（`pipe_detection_land_` 3・`pipe_detection_scope_` 3+1・`pipe_record_land_` 1）。直す 6 のうち行 an = `pipe_gate_health_` 3 と `gate_secs_only_fired_steps_carry_the_wall_clock` 1、行 ao = `pipe_main_same_tree_` の 2。snapshot の更新は行 an（gate の後の `pipe show` の行が変わるのは行 an）。write-set の外で検出線に触れるのは `tests/e2e/pipe.rs` の stub の定義（`verify-count.sh` の 1 行・定義のみ・触らない）と `crates/scribe2/src/pipe/cli/show.rs`（行 ak の写し `DetectionCopy` を読むだけ・消すのは gate の `Detection` と `DetectionSkip`・触らない）の 2 か所で、`pipe_confine_` の歯に検出線の字面は 0 本。
- 実装の決め（4 行に共通）: (i) 歯の helper は着地の後に `detection:spawned` が在れば同じ便の終えた語（measured / unmeasured / skipped）まで上限つきで待ち、無ければ待たない。行 ak の歯は record と写しの周を口の前後の差で測るが、event の `detection:measured` を 1 件と絶対値で測る assert は行 am の後は子の分で 2 件になる＝行 am が同じ便で「helper で子の終わりを待ってから口を撃ち、event も口の前後の差で測る」形に直す（口と子の置き場の別名は形 (4) の周の番号で分かれるので取り合わない・子を待たずに撃つ形は残さない）。land まで通り検出線を宣言する既存の歯（行 ak の 8 本を含む・done (12b) の列挙）は行 am の窓（gate も子も撃つ）で子の分だけ期待が動くので、同じ便で helper で待つ形に直す。(ii) 口の本体と子を起こす 1 か所は、段と event の種別を match の腕に書かない（`touches` を持つ他の doc の行の閉包を広げない）。(iii) 子の tmp worktree は着地した commit を detach で出すだけで ref を動かさない（列の次の便の CAS と交差しない）。
- 触らない: 穴 {teeth} と検出の母集団（§34・ADR-0052）・baseline と fail-fast（§33）・受付の値と式（§3・§31・`gate.slot_wait_s`）・遮断器の閾値（§32）・同じ木の主実測の record 1 本（§27）・R-C12-1 の極性と値・deny に昇格した行を同じ便で共通 verify へ移す規則（§5・ADR-0021 §2.4）・追随の再 gate の省略（pipeline.md §33）・`pipe report` の読み・台帳の記帳（close の 1 行だけ・FR50）。rules 行は足さない。
- 却下（ADR-0060 の選択肢）: 検出線を gate に残して時間だけ短くする（待ちが周ごとに残る・打ち切りの秒の rules 行が増える）／検出線を廃止する（C12.4 に反する）／runner に撃たせる（費用が runner の token へ移り、測定が自己申告になる）／dispatcher の周か常駐の process が着地済みの便を拾う（周が来ないと撃たれず、拾う走査と待ちの状態が増える）。本節で足す却下: 結果を bead の notes へ写す（C15・FR50）／rc 2 を自動で撃ち直す（撃ち直すのは人が同じ口を撃つ形だけ）／1 行で撃つ（diff が `gate.token_cap` を超える見込み）／「外す → 足す」の順（窓の便が測られない）／口を新しい subcommand にする（`PipeCommand` を `touches` に持つ行の閉包と外形 snapshot が動く・`--terminal-only` と同じ「着地をやり直さない口」の族に置く）。
- 歯（置き場は既存の file・新設の test file は作らない）:
  - **行 ak**（`tests/e2e/pipe/gate.rs`・検出線の stub と着地まで通す既存の fixture の隣・接頭辞 `pipe_landed_detection_`・base では `--detection-only` を断るので全部 RED）: (a) 測った周: 宣言の検出線が stub（argv と呼ばれた回数を記録し、`mutants-diff:` の判定行を出す）で、着地した diff が `crates/` 配下に触れる `Landed` の便に口を撃つと rc 0・stdout に `detection=measured`・`verify-main.jsonl` の `landed=<着地した sha>` を持つ `kind=detection` の record が口の前より 1 本多く `line=` が stub の判定行・stub の `--base` が着地した commit の親の sha（fixture は gate の base と親が異なる形で撃ち、gate の base を渡す変異を弁別する）で `--teeth` が契約の filter 語・共通 verify の stub は呼ばれない・event に `detection:measured` が 1 件・`pipe show --run` に stub の判定行と `secs=` の行・台帳の見張り（§37）の記録は 0 件。(b) 面の外: 着地した diff が docs だけの便は stub が呼ばれず、`reason=outside-scope` の skip record・理由の file・`detection:skipped`、`pipe show` の行は `detection-line: absent skipped=outside-scope`。(c) 測れなかった周: stub が rc 2 の便は stub が 1 回だけ呼ばれ、record の rc が 2・`detection:unmeasured`・show の行の末尾が `unmeasured=rc-2`、段は `Landed` のまま・main の sha は動かない。(d) 遮断器: 遮断器を閉じる `--rules` の fixture（§32 の歯と同じ形）で撃つと stub は呼ばれず `unmeasured=host-closed`。(d2) 出せない周: 主実測の置き場の別名の path に普通の file を置いて worktree を出せなくした便で撃つと stub は呼ばれず、`unmeasured=unprepared` の理由の file と `landed` 付きで `unmeasured=unprepared` を持つ record が 1 本増え、段は `Landed` のまま main の sha は動かない。(e) 的と純移動: 契約が的を持つ便は stub の argv に `--targets` と run dir の的の file が、純移動と証明された便は `--diff` と母集団の file が載る（gate と同じ付け足し）。(f) 断り: 段が `Gated` の便と、宣言に検出線の行が無い便は rc 1 で、record・写し・event がどれも増えない。外形 snapshot `pipe_external_form`（usage の land の switch）は同じ便で写す。
  - **行 al**（`tests/e2e/pipe/gate.rs` と `tests/e2e/pipe/land.rs`・接頭辞 `pipe_detection_single_shot_`）: (g) 共通 verify と契約 verify が緑で検出線だけ rc 2 の便の gate は PASS（lens の偽 PASS）・stub は 1 回だけ呼ばれ・record の rc は 2 で record の JSON に `retried` / `retried_from` の key が無い（key の不在を字面で測る＝欄を残す変異が落ちる・base では撃ち直して INCONCLUSIVE ＝ RED）。(h) 共通 verify が赤で検出線が rc 2 の便は FAIL で、evidence の赤い行の数に検出線を含まない。(i) main が `crates/` に触れて動いた便の追随で、便の diff の patch-id が前周と同じでも検出線がもう 1 回撃たれ、`carried` の record が無く、record の JSON に `patch_id` と `carried` の key が無い（key の不在を字面で測る・base では持ち越して撃たない ＝ RED）。§21 / §28 / pipeline.md §40 形 (b) の歯（接頭辞 `pipe_detection_retry_` / `pipe_gate_red_wins_over_detection_` / `pipe_detection_carry_` と、rc 2 を INCONCLUSIVE と測る歯・置き場は `crates/scribe2-boundary/tests/e2e/pipe/gate.rs` と `crates/scribe2-boundary/tests/e2e/pipe/land.rs`）は同じ便で消し、pipeline.md §40 形 (b) の散文には着地の errata を 1 行足す（規範は動かさない・`docs/design/pipeline.md` は行 al の write-set に在る）。`Detection` の持ち越しの variant と撃ち直しの enum を match するか引数に取る site は main 5864fcf の実測で `crates/scribe2/src/pipe/gate.rs` 3・`crates/scribe2/src/pipe/gate/verify.rs` 5・`crates/scribe2/src/pipe/gate/record.rs` 2・`crates/scribe2/src/pipe/land.rs` 1・`crates/scribe2/src/pipe/train.rs` 1 の 12 site で、land の子 module（verify / detection / finish）には無い＝行 al の write-set の 5 file で閉じる。
  - **行 am**（(k)〜(m) は `tests/e2e/pipe/gate.rs`・(o) は候補の木の fixture が在る `tests/e2e/pipe/land.rs`・接頭辞 `pipe_detection_after_landing_`・base では land が子を起こさないので `detection:spawned` が 0 件 ＝ RED）: (k) 同じ便の land は stub が終わる前に rc 0 で `Landed` を返す（stub は歯が置く解放の file を上限つきで待つ）。land が返った時点で event に `detection:spawned` が 1 件・終えた語は 0 件で、解放の後に helper が待つと `detection:measured` が 1 件と `landed` を持つ record が 1 本在る（gate が撃った ③ の record〔`landed` の無い `kind=detection`〕は行 am ではまだ 1 本残る・0 本になるのは行 an の (j)）。台帳の見張りの記録は 0 件のまま。(l) 検出線を宣言しない便の land は `detection:` で始まる detail を 1 件も書かない（従来の Landed の detail の並びのまま）。(m) main-red で終えた便は `detection:spawned` を持たない。(o) 候補の木で着地した後続は自分の run dir に `landed` を持つ record を 1 本持ち、stub の `--base` が自分の squash commit の親。runner の雛形の歯（`headless_runner_prompt_external_form` の snapshot と、雛形の検出線の行を 1 本と数える `headless_runner_box_claude_is_one_job_and_the_prompt_names_the_detector`）は同じ便で新しい字面へ写す。
  - **行 an**（(j)(n) は `tests/e2e/pipe/gate.rs`・接頭辞 `pipe_detection_off_gate_`・base では ③ を撃つ ＝ RED）: (j) 検出線を宣言した便の gate は stub を 1 回も呼ばず、`verify.jsonl` に `kind=detection` の record が無く、gate の後の `pipe show` に検出線の判定行が無い（base では ③ を撃つ ＝ RED）。(n) main が `crates/` に触れて動いた便の追随の再 gate も stub を呼ばない。docs だけの動きは再 gate を省く（pipeline.md §33 の既存の歯が緑のまま）。消す歯 23 本と直す歯 4 本は形 (13) の便の割り当てのとおりで、`pipe_record_show_external_form` の snapshot は検出線の判定行の無い形で更新する。型と欄は残す（`cli/step.rs` と `land.rs` の `Gate` の構築点は触らない）。1 便の diff は .603 の 2 回目で 178302 byte（消す歯 89 KB + src 53 KB + 他）で cap 150000 byte を超えたので、行 an と行 ao の 2 行に割った（行 an ≈ 95 KB・行 ao ≈ 75 KB の見込み）。
  - **行 ao**（(p) は `tests/e2e/pipe/gate.rs`・接頭辞 `pipe_detection_off_main_`・base では主実測が ③ を撃つ ＝ RED）: (p) 検出線を宣言した便を land まで通すと、主実測の record に `landed` の無い `kind=detection` が 0 本で（木が違う周も ①②④ だけ）、候補の木の後続の record も同じ（base では主実測が ③ を撃つか `skipped=detection` を書く ＝ RED）。同じ木の周は record 1 本のまま（`pipe_main_same_tree_writes_one_record_and_fires_nothing` が残って測る）。消す歯 8 本と直す歯 2 本は形 (13) の便の割り当てのとおり。型 `Detection` と DetectionSkip の SameTree と `Gate` の欄を消し、追随の面の判定を形 (10) の 1 役にする。`pipe/land.rs` の in-file の歯（`SameTree` の fixture）は字面の置き換えだけで base でも通るので、flip-check の retroactive の札を歯の区間の行頭に置く（core-boundary.md §9 の 10 の型）。
- 後続: 同時に走る着地後の検出の本数と、着地のたびの冷えた build の秒を、行 an の着地の後に実測する（上限の rules 行が要るかは実測を見て user が裁定・ADR-0050）。`pipe report` に着地後の検出を載せるかは別の便。行 an の着地は PATH の器の入れ替えを伴う（gate が ③ を撃たなくなるのは入れ替えた後から）。
...
### 44.2 errata（現物との差・行 al・規範は上の §44 のまま）
...
- 形 (9) の gate の段の列: 行を撃つ 1 本は段の列を引数に取り、gate は `CHECKS` から ③ を除いた列を組む関数を、land の主実測と候補の木は `CHECKS` を渡す（撃つ実装は 1 本のまま）。Detection の Run の周は写しの検出線も的の file も母集団の diff も組まず、写しの置き場も作らない。追随の再 gate の Detection の Skip の周は従来どおり ③ の位置に skip record を 1 本置く（`Skipped` の構築を消すのは行 ao）。gate の記録の側の「検出線 ∧ rc 2」の述語と箱の中の oom の吸収は、gate の列に ③ が無いので当たらないまま残る（型と一緒に行 ao で畳む）。
...
### 44.5 errata（現物との差・行 ao・規範は上の §44 のまま）

- 形 (9) の段の列: 行 an の gate の列を組む関数を主実測と候補の木も通す（行を撃つ 1 本の受付なしの口がその列を渡す）。`CHECKS` は全変種の列のまま残る。候補の木は後続ごとの段を持たず、先端の木の共通 verify と契約 verify だけを撃つ。
- 形 (9) の skip record の材料: 構築の口は追随の再 gate と主実測の 2 つだけになり、理由の欄は record の字面を持つ（再 gate は面の外・主実測は same-tree の 1 語を record の側の定数で書く＝record の形は不変）。理由の enum は面の外の 1 値だけが残る（再 gate と着地後の検出の面の外が同じ字面で持つ）。record 列を組む 1 本は skip record を末尾に置く形に畳んだ（③ の位置に挟む周が無くなった）。
...
req = ["FR8", "NFR3"]
section = "3"
write-set = ["rules/manifest.toml", "crates/scribe2/src/rules/mod.rs", "crates/scribe2-boundary/tests/e2e/rules.rs", "crates/scribe2-boundary/tests/e2e/snapshots/e2e__rules__rules_external_form.snap", "crates/xtask/src/limits.rs", "crates/xtask/src/check.rs", "crates/xtask/src/check_facts.rs", "crates/xtask/src/check_tests.rs", "+.config/nextest.toml", "docs/design/gate-cost.md"]
...
req = ["FR46", "NFR3"]
section = "13"
write-set = ["crates/scribe2/src/pipe/confine.rs", "crates/scribe2/src/headless/runner.rs", "crates/scribe2/src/headless/lens.rs", "crates/scribe2-boundary/tests/e2e/headless.rs", "crates/scribe2-boundary/tests/e2e/snapshots/e2e__headless__headless_external_form.snap", "docs/design/gate-cost.md"]
...
req = ["FR8", "NFR3"]
section = "14"
write-set = ["crates/scribe2/src/pipe/gate.rs", "crates/scribe2/src/pipe/gate/verify.rs", "crates/scribe2/src/pipe/gate/record.rs", "crates/scribe2/src/pipe/move_proof.rs", "crates/xtask/src/mutantsdiff.rs", "crates/scribe2-boundary/tests/e2e/pipe/gate.rs", "docs/design/gate-cost.md"]
...
req = ["FR8", "FR22"]
section = "15"
write-set = ["crates/scribe2/src/pipe/gate/record.rs", "crates/scribe2/src/pipe/gate.rs", "crates/scribe2/src/pipe/cli/show.rs", "crates/scribe2-boundary/tests/e2e/pipe/gate.rs", "crates/scribe2-boundary/tests/e2e/snapshots/e2e__pipe__gate__pipe_record_show_external_form.snap", "docs/design/gate-cost.md"]
...
req = ["FR46"]
section = "20"
write-set = ["crates/scribe2-boundary/tests/e2e/seat.rs"]
...
req = ["FR9", "AC3"]
section = "21"
write-set = ["crates/scribe2/src/pipe/gate/verify.rs", "crates/scribe2/src/pipe/gate/record.rs", "crates/scribe2-boundary/tests/e2e/pipe/gate.rs"]
...
req = ["NFR6"]
section = "23"
write-set = ["crates/scribe2-boundary/tests/e2e/pipe/land.rs", "crates/scribe2-boundary/tests/e2e/fleet.rs"]
...
req = ["FR68", "FR39"]
section = "24"
write-set = ["rules/manifest.toml", "crates/scribe2/src/rules/mod.rs", "crates/scribe2-boundary/tests/e2e/rules.rs", "crates/scribe2-boundary/tests/e2e/snapshots/e2e__rules__rules_external_form.snap", "docs/design/rules-manifest.md", "docs/design/gate-cost.md", "crates/scribe2/src/pipe/refuse.rs", "crates/scribe2/src/pipe/cli/intake.rs", "crates/scribe2-boundary/tests/e2e/pipe.rs", "crates/scribe2-boundary/tests/e2e/pipe/intake.rs"]
...
req = ["NFR6"]
section = "25"
write-set = ["crates/scribe2/src/pipe/confine.rs", "crates/scribe2-boundary/tests/e2e/pipe/gate.rs", "crates/scribe2-boundary/tests/e2e/pipe.rs"]
...
req = ["FR8"]
section = "26"
write-set = ["crates/scribe2/src/pipe/gate/verify.rs", "crates/scribe2/src/pipe/gate/record.rs", "crates/scribe2/src/pipe/cli/show.rs", "crates/scribe2-boundary/tests/e2e/pipe/gate.rs", "crates/scribe2-boundary/tests/e2e/snapshots/e2e__pipe__gate__pipe_record_show_external_form.snap"]
...
req = ["FR34", "FR12", "FR50"]
section = "27"
write-set = ["crates/scribe2/src/pipe/land/verify.rs", "crates/scribe2/src/pipe/gate/record.rs", "crates/scribe2-boundary/tests/e2e/pipe/gate.rs", "crates/scribe2-boundary/tests/e2e/pipe/land.rs", "docs/design/gate-cost.md"]
...
req = ["FR9", "AC3", "FR14"]
section = "28"
write-set = ["crates/scribe2/src/pipe/gate.rs", "crates/scribe2-boundary/tests/e2e/pipe/gate.rs"]
...
req = ["FR9", "FR14", "NFR4"]
section = "29"
write-set = ["crates/scribe2/src/pipe/gate/lens.rs", "crates/scribe2/src/pipe/gate/findings.rs", "crates/scribe2/src/pipe/gate.rs", "crates/scribe2-boundary/tests/e2e/pipe/gate.rs"]
...
req = ["NFR6", "FR46"]
section = "31"
write-set = ["crates/scribe2/src/pipe/admission.rs", "crates/scribe2/src/pipe/gate/verify.rs", "crates/scribe2/src/pipe/declaration.rs", "crates/scribe2/src/fleet/wait.rs", "crates/xtask/src/mutantsdiff.rs", "crates/xtask/src/main.rs", ".vessel.toml", "crates/scribe2-boundary/tests/e2e/pipe/gate.rs", "docs/design/gate-cost.md"]
...
req = ["NFR6", "FR46"]
section = "35"
write-set = ["rules/manifest.toml", "crates/scribe2-boundary/tests/e2e/rules.rs", "docs/design/gate-cost.md"]
...
req = ["NFR6", "FR46"]
section = "36"
write-set = ["crates/scribe2-boundary/tests/e2e/main.rs", "crates/scribe2-boundary/tests/e2e/pipe.rs", "docs/design/gate-cost.md"]
...
req = ["FR50", "NFR6"]
section = "37"
write-set = ["crates/scribe2-boundary/tests/e2e/main.rs", "crates/scribe2-boundary/tests/e2e/pipe.rs", "crates/scribe2-boundary/tests/e2e/pipe/dispatch.rs", "docs/design/gate-cost.md"]
...
req = ["NFR6", "FR46"]
section = "38"
write-set = ["crates/scribe2/src/pipe/confine.rs", "crates/scribe2/src/pipe/stop.rs", "crates/scribe2-boundary/tests/e2e/main.rs", "crates/scribe2-boundary/tests/e2e/pipe/gate.rs", "crates/scribe2-boundary/tests/e2e/pipe/stop.rs", "docs/design/gate-cost.md"]
...
req = ["FR9", "NFR1"]
section = "41"
write-set = ["crates/scribe2/src/pipe/gate/lens.rs", "crates/scribe2/src/pipe/gate.rs", "crates/scribe2/src/pipe/gate/record.rs", "crates/scribe2-boundary/tests/e2e/pipe/gate.rs", "docs/design/gate-cost.md"]
...
req = ["FR9", "NFR1"]
section = "42"
write-set = ["crates/scribe2/src/pipe/gate/lens.rs", "crates/scribe2/src/pipe/gate.rs", "crates/scribe2-boundary/tests/e2e/pipe/gate.rs", "docs/design/gate-cost.md"]
verify = ["cargo nextest run -p scribe2 --test e2e --no-tests=fail pipe_gate_elide_", "cargo nextest run -p scribe2 --lib --no-tests=fail gate_elide_dir_pairs_"]
size = "S"
done = "(1) hunk の本文を - の連続 k 行の直後の + の連続 k 行の段に切り、段の間と前後の context を許し、段ごとに - の各行が置換で 1 字以上変わり置換後の列が + の列と順序も本数も同じなら hunk 全体を 1 行の印に畳み（件数は全段の合計・本数の違いは列の相等に含まれ別の条件を持たない）、- の連続の直後が context か段の本数が違えば逐語のまま (2) measure が HEAD の tracked path の列を読めた周だけ渡し（読めない周と空の周は無し）、pure な 1 本は列が無い周に dir の対の導出を丸ごと行わず、列が在る周は rename の対ごとに末尾の共通 component を剥がした prefix の対を長い方から見て、HEAD に配下の path が無く配下の rename が全部同じ dir へ行く間だけ dir の対として足し、file の対と同じ列で長い旧 path から順に置換する (3) 通知の elided=<hunk 数>/<行数> と bytes= と diff_bytes の字面と意味は §41 のまま (4) §41 の歯 8 本は 1 字も変えず緑のまま、新しい歯 (i) 2 段の hunk は畳む・(j) 段の本数が違えば逐語・(k) 空になった dir を名指す置換は畳む・(l) 配下に path が残る dir は対にしない・(m) 配下の rename が別の dir へ行く dir は対にしない、の 5 本と、unit の (n) HEAD の列が無い周は rename の対が在っても dir の対が 0（列が在る対照は対が在る）が緑で、段の切り分けを 1 塊に戻すと (i) が、dir の対の導出を外すと (k) が、空の条件を外すと (l) が、一貫の条件を外すと (m) が、列が無い周を空の列として扱うと (n) が落ち、(j) と (a)〜(h) はどの変異でも落ちない回帰の歯"

[[contract]]
id = "aj"
title = "便ごとの token 消費の検出線 — rules 行 R-C6-1（値 25000000・裁定 user 2026-09-23T07:16Z）を足し、pipe show --run が便の消費の event の 4 値の和を読み、閾値以上の便にだけ判定行 cost-ceiling: over を 1 行出す（閾値を読めない周は unmeasured・断らない・停止は別の裁定）"
req = ["NFR1", "FR21"]
section = "43"
write-set = ["rules/manifest.toml", "crates/scribe2/src/rules/mod.rs", "crates/scribe2/src/pipe/cli/show.rs", "crates/scribe2-boundary/tests/e2e/rules.rs", "crates/scribe2-boundary/tests/e2e/snapshots/e2e__rules__rules_external_form.snap", "crates/scribe2-boundary/tests/e2e/pipe/spawn.rs", "docs/design/rules-manifest.md", "docs/design/gate-cost.md"]
verify = ["cargo nextest run -p scribe2-boundary --test e2e --no-tests=fail run_cost_ceiling_", "cargo nextest run -p scribe2-boundary --test e2e --no-tests=fail rules_embedded_manifest_declares_run_token_ceiling_", "cargo nextest run -p scribe2-boundary --test e2e --no-tests=fail rules_external_form"]
size = "S"
done = "(1) 埋め込み manifest に行 R-C6-1 が在り、値 25000000・値の形 Int・発効・裁定 id user 2026-09-23T07:16Z・裁定日 2026-09-23 を持ち、kind は RuleKind の新しい variant 1 つで ALL の GateTokenCap の直後に在って字面から引け、rules-manifest.md の §4.1 の表に行が在り、同じ § の置かない行から R-C6-1 が消え、cargo xtask check の rules-parity が doc-only=0 ids=- を出す (2) 便の消費の event の in / out / cache_read / cache_create を event をまたいで飽和加算した和が閾値以上の便にだけ、pure な 1 本が cost-ceiling: over total=<和> limit=<閾値> events=<event 数> の 1 行を返し、閾値未満の便と event 0 件の便は行を返さない (3) pipe show --run が消費の行の後にその 1 行を置き、閾値は --rules か埋め込みの行 R-C6-1 から読み（行の id は show.rs の const 1 つ）、event が 1 件以上在るのに閾値を読めない周（行が無い・不発効）は cost-ceiling: unmeasured events=<n> を出し、1 行目と既存の行と rc は変わらず、show は event を書かない (4) 受付・spawn・gate・land・driver は行 R-C6-1 を読まず、和が閾値ちょうどの便も land が rc 0 で Landed に着く (5) run_cost_ceiling_ の歯が、和が 25000000 ちょうどの便で over の行 1 本・24999999 の便で 0 本（どちらも Landed・4 値はどれも 0 でない・event 2 件）と、行の無い fixture と不発効の fixture で unmeasured の行 1 本・event 0 件の便で 0 本を測り、4 値のどれかを落とす実装・最後の event だけを数える実装・> で比べる実装で落ち、rules_embedded_manifest_declares_run_token_ceiling_ の歯が行の 5 値と位置を測り、行数と kind 数の pin 2 か所と rules_external_form の snapshot が land 時の現物に +1 で写る"

[[contract]]
id = "ak"
title = "着地後の検出の口 — pipe land --run <id> --detection-only が Landed の便の着地した commit を tmp worktree に出し、検出線の行だけを gate と同じ構築・受付札・箱・遮断器で 1 回撃ち、verify-main.jsonl に landed 付きの record・§15 の置き場に写しと理由の file・RunDone stage=Landed に detection の 1 語を残す（人が撃つ形・gate と land の段は変えない）"
req = ["NFR6", "FR46", "FR50"]
section = "44"
write-set = ["+crates/scribe2/src/pipe/land/detection.rs", "crates/scribe2/src/pipe/land.rs", "crates/scribe2/src/pipe/land/verify.rs", "crates/scribe2/src/pipe/gate.rs", "crates/scribe2/src/pipe/gate/verify.rs", "crates/scribe2/src/pipe/gate/record.rs", "crates/scribe2/src/pipe/cli/step.rs", "crates/scribe2/src/pipe/cli/args.rs", "crates/scribe2/src/pipe/cli.rs", "crates/scribe2-boundary/tests/e2e/pipe.rs", "crates/scribe2-boundary/tests/e2e/pipe/gate.rs", "crates/scribe2-boundary/tests/e2e/snapshots/e2e__pipe__pipe_external_form.snap", "docs/design/gate-cost.md"]
verify = ["cargo nextest run -p scribe2-boundary --test e2e --no-tests=fail pipe_landed_detection_", "cargo nextest run -p scribe2-boundary --test e2e --no-tests=fail pipe_external_form"]
size = "M"
done = "(1) pipe land が値の無い switch --detection-only を受け、段が Landed の便だけを通し、着地した sha を記録から読み、宣言の写しに検出線の行が無い便と Landed でない便は何も書かずに rc 1 で断り、stdout は run=<id> detection=<measured|unmeasured|skipped> の 1 行で rc 0（書けない周だけ rc 2）、本体は + の file に在って land.rs には mod 宣言だけが増える (2) 着地した commit を主実測と同じ置き場の別名に detach で出し、{base} は着地した commit の親の sha・{teeth} は契約の写しから・行は凍結した宣言の写しの検出線で、的と純移動の付け足しと写しの周の番号は gate と同じ 4 本（gate の材料でなく置き場と便 id の対を取る形）を通り、③ の行だけを撃つ入口が gate と同じ受付札・縮退・箱・遮断器の行の手を通して 1 回だけ撃ち（①②④ と撃ち直しは無い）、撃った後に worktree を畳む (3) 着地した commit と親の path が DETECTION_SCOPE に触れない周は撃たず reason=outside-scope の skip record を残し、path を読めない周は撃つ (4) 撃った便自身の run dir の verify-main.jsonl に kind=detection・tree・任意 field landed=<着地した sha> の record を、撃った周は写しの検出線の行ごとに 1 本、撃たなかった周と撃てなかった周（面の外・遮断器が閉じた・worktree か親を出せない）は理由を持つ 1 本、どの周も 1 本以上追記し（親か木を読めない周の tree は主実測の unknown の語）、写しを §15 の置き場に置き、測れなかった周（rc が 0 と 1 以外・遮断器が閉じた・worktree か親を出せない）と撃たなかった周はその置き場に unmeasured=<理由> か skipped=outside-scope の理由の file を置き、RunDone stage=Landed の detail に detection:measured / detection:unmeasured / detection:skipped のどれか 1 語を 1 件記し、台帳 client を呼ばない (5) pipe show --run の読み手は変わらず、理由の file の在る周だけ写しの行の末尾に理由の 1 語が付き、理由の file の無い周の字面は 1 字も変わらない 歯: pipe_landed_detection_ の歯が、測った周（record +1・line= が判定行・--base が親〔gate の base と親が異なる fixture で弁別〕・--teeth が契約の語・共通 verify は撃たない・event 1 件・show の行・台帳の見張り 0 件）、面の外の周、rc 2 の周（stub 1 回・unmeasured=rc-2・段と main は不変）、遮断器が閉じた周、worktree か親を出せない周（record 1 本・unmeasured=unprepared・段と main は不変）、的と純移動の付け足し、Landed でない便と検出線の無い便の断りを測り、外形 snapshot pipe_external_form が usage の land の switch を写す"

[[contract]]
id = "al"
title = "gate の中の検出線を 1 回撃ちにする — 検出線の rc 2 の撃ち直し（§21）と rc 2 を INCONCLUSIVE に倒す判定（§5・§28）と持ち越し（patch-id・Detection::Carry）を消す（③ はまだ gate が撃つ・行 am の前段）"
req = ["FR8", "FR34"]
section = "44"
write-set = ["crates/scribe2/src/pipe/gate.rs", "crates/scribe2/src/pipe/gate/verify.rs", "crates/scribe2/src/pipe/gate/record.rs", "crates/scribe2/src/pipe/land.rs", "crates/scribe2/src/pipe/train.rs", "crates/scribe2-boundary/tests/e2e/pipe/gate.rs", "crates/scribe2-boundary/tests/e2e/pipe/land.rs", "docs/design/gate-cost.md", "docs/design/pipeline.md"]
verify = ["cargo nextest run -p scribe2-boundary --test e2e --no-tests=fail pipe_detection_single_shot_"]
size = "M"
depends = ["ak"]
done = "(6) gate の行の手に検出線の撃ち直しが無く、record の JSON に retried と retried_from の key が無い（key の不在を字面で測る） (7) 判定の順に検出線の rc 2 の段が無く、検出線の rc 2 は赤にも INCONCLUSIVE にも数えず record の rc 2 のまま残り、検出線の rc 1 は赤のままで段①・箱の中の死・遮断器の順は動かない (8) Detection の Carry と持ち越しの判定と patch-id の測りと持ち越しの record が無く、record の JSON に patch_id と carried の key が無く（key の不在を字面で測る）、追随と候補の木の段は面の判定だけで撃つか省くかを決め、pipeline.md §40 形 (b) の散文に着地の errata を 1 行足す（規範は動かさない） 歯: pipe_detection_single_shot_ の歯が、検出線だけ rc 2 の便の gate が PASS で stub が 1 回だけ呼ばれ record の rc が 2 で JSON に retried と retried_from の key が無い・赤い共通 verify と rc 2 の検出線の便が FAIL で evidence の赤に検出線を含まない・main が crates に触れて動いた追随で patch-id が同じでも検出線がもう 1 回撃たれ持ち越しの record が無く record の JSON に patch_id と carried の key が無い、を測り、撃ち直し・rc 2 の INCONCLUSIVE・持ち越しを測る既存の歯は同じ便で消える"

[[contract]]
id = "am"
title = "land が Landed の event の後に行 ak の口を列と同じ起こし方で切り離して起こす（待たない・detection:spawned / unspawned・runner の雛形の 1 行を直す・③ はまだ gate も撃つ＝形 (11)(12)）"
req = ["FR8", "NFR6", "FR50"]
section = "44"
write-set = ["crates/scribe2/src/pipe/land.rs", "crates/scribe2/src/pipe/land/verify.rs", "crates/scribe2/src/pipe/land/detection.rs", "crates/scribe2/src/pipe/land/finish.rs", "crates/scribe2/src/pipe/train.rs", "crates/scribe2/src/pipe/queue.rs", "crates/scribe2/src/pipe/dispatch.rs", "crates/scribe2/src/pipe/cli/step.rs", "crates/scribe2/src/headless/runner.txt", "crates/scribe2-boundary/tests/e2e/pipe/gate.rs", "crates/scribe2-boundary/tests/e2e/pipe/land.rs", "crates/scribe2-boundary/tests/e2e/headless.rs", "crates/scribe2-boundary/tests/e2e/snapshots/e2e__headless__headless_runner_prompt_external_form.snap", "docs/design/gate-cost.md"]
verify = ["cargo nextest run -p scribe2-boundary --test e2e --no-tests=fail pipe_detection_after_landing_", "cargo nextest run -p scribe2-boundary --test e2e --no-tests=fail headless_runner_prompt_external_form", "cargo nextest run -p scribe2-boundary --test e2e --no-tests=fail headless_runner_box_claude_is_one_job_and_the_prompt_names_the_detector"]
size = "M"
growth = ["crates/scribe2/src/pipe/dispatch.rs:5"]
depends = ["ak", "al"]
done = "(11) finish が Landed の event の後・worktree の退避と終端の前に、宣言の写しに検出線の行が在る便だけ列と同じ起こし方の 1 本で行 ak の口を --state-dir と --repo（と受けた --rules）付きの子 process として起こして待たず、起こせた周は detection:spawned・起こせなかった周は detection:unspawned と理由の file unmeasured=unspawned と stderr の 1 行を残して rc と着地を変えず、main-red と main-unmeasured の周は起こさず、候補の木の後続も自分の finish から起こし、land の材料に --rules の path の欄が 1 つ増える。理由の語 unspawned と理由の file を書く手と検出線の宣言を読む手は行 ak の本体 crates/scribe2/src/pipe/land/detection.rs（land.rs の pub(in crate::pipe) の mod・main ec16e0c の実測: 口は pub(in crate::pipe) の detect と Detect、理由の語 unprepared / host-closed / rc-<rc> は private の定数と関数、理由の file を書く手は private の unfired、宣言の検出線の行の有無は land.rs の detection_needed）に在るので、detection.rs を write-set に持ち、unspawned はその閉じた語の列に足し、理由の file を書く手は pub(super) に上げて finish から呼ぶ（新しい書き手を finish に増やさない・C2） (12) runner の雛形の検出線の行が着地の後に器が撃つ形になり、runner は撃たず禁じる語列は変わらない (12b) gate・主実測・候補の木の段の列と ③ の record は本行では変えない（外すのは行 an）。ただし land まで通り検出線を宣言する既存の歯（main 6efec0d の実測: tests/e2e/pipe/gate.rs の pipe_detection_land_ 3・pipe_detection_scope_ 3・pipe_main_same_ 3 の 9 本と、行 ak の歯 pipe_landed_detection_ 7 本と pipe_record_land_skipped_detection_has_no_line 1 本（landed_run の fixture が検出線を宣言した toy を着地させ、その後に口を手で撃つ）の 8 本と、tests/e2e/pipe/land.rs の pipe_follow_docs_ / pipe_follow_self_ / pipe_detection_scope_ / pipe_detection_single_shot_ のうち検出線の toy を land まで通す歯）は、着地のたびに子が stub を 1 回呼び landed 付きの record を 1 本足すので、実装の決め (i) の helper で子の終わりを待ち、期待を「gate の分 + 着地後の 1 回」と「主実測の record + landed 付き 1 本」に直す（同じ便・待たないと flaky・行 an で消す歯も本行の窓では待って緑にする） 歯: pipe_detection_after_landing_ の歯が、land が stub の終わる前に rc 0 で Landed を返し spawned が 1 件で終えた語が 0 件・解放の後に measured が 1 件と landed を持つ record が 1 本（gate の ③ の record は本行ではまだ 1 本残る）・台帳の見張り 0 件・検出線を宣言しない便は detection の detail を書かない・main-red の便は spawned を持たない・候補の木の後続は自分の run dir に landed を持つ record を持ち --base が自分の squash commit の親、を測り、runner の雛形の snapshot と検出線の行の歯が新しい字面で緑 (12c) verify の 3 本目の歯は雛形の検出線の行を 1 本と数える歯で、雛形の新しい字面（着地の後に器が撃つ）に合わせて同じ便で写す（§44 歯 (o)）"

[[contract]]
id = "an"
title = "検出線を gate から外す — gate の段の列を ①②④ にし（型と欄は残す）、③ を gate が撃つことを測る既存の歯 23 本を消し 4 本を直し pipe show の snapshot を更新する（行 am の後・形 (9) と (13) の gate 側）"
req = ["FR8", "NFR6", "FR50"]
section = "44"
write-set = ["crates/scribe2/src/pipe/gate/verify.rs", "crates/scribe2/src/pipe/gate/record.rs", "crates/scribe2-boundary/tests/e2e/pipe/gate.rs", "crates/scribe2-boundary/tests/e2e/pipe/land.rs", "crates/scribe2-boundary/tests/e2e/snapshots/e2e__pipe__gate__pipe_record_show_external_form.snap", "docs/design/gate-cost.md"]
verify = ["cargo nextest run -p scribe2-boundary --test e2e --no-tests=fail pipe_detection_off_gate_"]
size = "M"
depends = ["am"]
done = "(9a) gate の段の列に ③ が無く（gate は ①②④・Detection::Run の周も撃たず写しも書かない・型 Detection と DetectionSkip と Gate の欄と cli/step.rs と land.rs の構築点は不変）、gate の段の列は &[Check] 型の const でなく CHECKS から ③ を除いて組む関数で（enum-slices の門が全変種の const slice を求める・CHECKS は不変）、cargo xtask check が緑、主実測と候補の木の段は変わらない (13a) ③ を gate が撃つことを測る既存の歯が消え（消す 23 本と直す 4 本は §44 の形 13 の便の割り当て・main d11b83f）、pipe show の外形の snapshot は検出線の判定行の無い形で更新され、tests/e2e/pipe.rs の stub の定義と cli/show.rs は触らない 歯: pipe_detection_off_gate_ の歯が、検出線を宣言した便の gate が stub を 1 回も呼ばず verify.jsonl に kind=detection が無く pipe show に判定行が無い・main が crates に触れて動いた追随の再 gate も stub を呼ばない、を測る"

[[contract]]
id = "ao"
title = "検出線を主実測と候補の木から外し Detection と gate の材料の欄と DetectionSkip::SameTree を消す — 既存の歯 8 本を消し 2 本を直す（行 an の後・形 (9) の残りと (10) と (13) の残り）"
req = ["FR8", "NFR6", "FR50"]
section = "44"
write-set = ["crates/scribe2/src/pipe/gate.rs", "crates/scribe2/src/pipe/gate/verify.rs", "crates/scribe2/src/pipe/gate/record.rs", "crates/scribe2/src/pipe/land.rs", "crates/scribe2/src/pipe/land/verify.rs", "crates/scribe2/src/pipe/train.rs", "crates/scribe2/src/pipe/cli/step.rs", "crates/scribe2-boundary/tests/e2e/pipe/gate.rs", "crates/scribe2-boundary/tests/e2e/pipe/land.rs", "docs/design/gate-cost.md"]
verify = ["cargo nextest run -p scribe2-boundary --test e2e --no-tests=fail pipe_detection_off_main_"]
size = "M"
depends = ["an"]
