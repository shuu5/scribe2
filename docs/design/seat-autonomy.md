...
- `seat tick --target T --wm-dir DIR` … 裁定 (c) の 4 条件を**順序固定の AND** で評価し、成立時だけ注入する。判定は enum `TickDecision::{Inject, Noop(NoopReason), Error(String)}` で持ち bool にしない（憲法 C11）。
  - **順序** = **退避物の走査** → 鮮度 → pane 取得 → **context** → idle → 未 consumed WM → cycle lock。**最初に立たなかった条件**を理由にする（`heartbeat-fresh` / `pane-missing` / `busy` / `wm-unconsumed` / `wm-unreadable` / `cycle-live`）。順序は load-bearing で、**退避物の無い fresh の周は tmux を 1 度も叩かない**（生きている席を毎周 capture しない）。**自席の未 consumed 退避物が在る周は鮮度 gate を飛ばす**（`s2-07l.105`・裁定 (a)・user 直命 2026-09-11「流石に長すぎだろ」）: 退避を終えた席は heartbeat を直前に打っていることが多く、退避物の存在は席の「作り直してよい」の申告なので、鮮度を理由に pane も見ずに noop すると退避から作り直しまでが最大 `seat.tick_stale_s`（40 分）に延びる（admin2 で実測: 退避完了時 age 588 s → 約 30 分 idle）。飛ばすのは自席の名乗り（裁定 (c)）が在る周だけで、読めない周は「在る」に読み替えず従来どおり鮮度から。飛ばした周もそれ以降の条件（pane → context → idle → lock）は不変＝busy の席・lock が live な周には送らない。理由の字面は順序の証拠でもある＝入れ替えると同じ条件でも別の理由が出る。
  ...
  - **裁定 (e) idle**（**supersede・ADR-0015 / [seat-state.md](./seat-state.md)**: 席の状態は hook の打刻で typed に持ち、pane の字面は判定入力にしない。以下の字面規則は `s2-07l.95` が land するまでの現行実装の記録）= 最後の prompt 行の右が空 ∧ **探索域**に**走行中の印**が無い。
