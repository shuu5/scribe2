...
## 15. 退避の閾値を窓ごとに持ち、7 日窓とモデル別窓は強制 cycle にする（契約表の行 g〜k・`s2-07l.434`）

- 何が起きているか（planner 席 2026-09-17・verified・`s2-07l.434`）: session 用の閾値は rules 行 `R-C9-1`（kind `AccountSelection`・Int）の 1 値で、選定（`fleet/select.rs` の `standing`）も席の逼迫度（`seat/tick/account.rs` の `reading`）も「数える窓の最大の使用率 ≥ その 1 値」で判定する＝3 つの窓（`WindowKind` の 5 時間・7 日・モデル別 7 日）に同じ値が効く。同時に多数流した agent の返答がまとまって戻り、5 時間窓が閾値を越えてから上限に当たるまでに退避が間に合わなかった。user 裁定 2026-09-17T03:28Z / 03:50Z（逐語は台帳 `s2-07l.434`）の要旨: 5 時間窓は 85 のまま・7 日窓とモデル別窓は全量が大きいので 95 まで使う・ただし 95 の 2 窓は合図を受けた席がそこから新しい作業を始めないことを器が担保する（強制 cycle）。窓別の口が入るまでの特例として、別の便（`s2-07l.447`・[rules-manifest.md](./rules-manifest.md) §13・user 裁定 2026-09-17T07:30Z）が `R-C9-1` の 1 値を 85 から 95 へ書き換えて先に着地する（全部の窓に 95 が効く）＝本 § の行 k はその後の manifest を base に読み、`R-C9-1` を 5 時間窓の 85 に戻す。口座の選定と退避の合図の結び方は [account-autonomy.md](./account-autonomy.md) §5 のまま。
- 形（判定入力は typed な値だけ・C3.3）:
  1. **rules 行は 3 つ足す**（C1 / C5・裁定 id = user 2026-09-17T03:50Z・`RuleKind` に variant 3 つ・宣言順は `AccountSelection` の直後）: `R-C9-1.seven-day`（Int・95）・`R-C9-1.seven-day-model`（Int・95）・`R-C9-1.forced-windows`（List・値は `WindowKind` の `as_str` の字面 = `seven_day` と `seven_day_model`）。同じ便（行 k）が `R-C9-1` の行を**書き換える**: 値を 85 に戻し（base は特例の便が置いた 95）、`ruling` を user 2026-09-17T03:28Z（5 時間窓 = 85 の裁定）・`ruled_at` をその日付にする。読みは「全部の窓」から「5 時間窓」に狭まる＝7 日窓とモデル別窓の 95 は新しい 2 行が持ち、特例の全窓 95 はこの便で終わる（行の値と裁定 id を pin する既存の歯と `rules get R-C9-1` を含む外形 snapshot も同じ便で動く）。id は既存の compound 行（`R-C13-1.per-pr`）と同じ形なので、憲法 §3 との突合（`rules-parity`）は `R-C9-1` に畳まれて動かない。「どの窓が強制か」も規則の値なので行に持つ（code の match に埋めない・C1）。
  2. **窓別の閾値の型**: `fleet/select.rs` に閉じた型 `WindowThresholds`（窓ごとの閾値 3 値 + 強制の窓の集合）を置く。口は 4 つで全部この型が持つ: **窓から閾値を引く口**（`WindowKind` の網羅 match の 1 関数）・**窓が強制かを問う口**・**全部の窓に同じ値を入れる構築子**（強制の窓は空・便用が渡す `LIMIT_PCT` と歯の fixture 用）・**4 行の値から組む構築子**（閾値 3 値と強制の窓の語の列を受け、語が `WindowKind` の `parse` を通らなければ断る pure 関数・manifest は読まない）。`Input` の `threshold_pct` と、席の側の運び（tick の `Seen`・launch / relaunch の `Request`・`choose` の引数）をこの型に差し替える。**land は 2 段**で、選定と席の逼迫度が別々の値を読む中間状態を作らない: 行 g = 型と 4 つの口と運びの差し替えと窓別の比較（`standing` と `reading`）を入れ、**全部の構築点が従来の 1 値を全窓同値の構築子で入れる**（挙動不変）／行 k = rules 行 3 つを足し、選定側と席側の読み手の全部を**同じ便で** 4 行の構築子へ切り替える（挙動が変わるのはこの 1 便・rules 行は読み手と同時に land する＝読み手の無い行を main に置かない・C10.3）。`standing` の順序（除外 → 測れない → 当たっている → 閾値）は不変で、4 つ目の判定だけが「逼迫度 ≥ 1 値」から「数える窓のどれかが自分の窓の閾値以上」に変わる（並べる鍵の逼迫度 = 数える窓の最大の使用率は不変）。manifest からの読みは 2 面の既存の読み手（`fleet/cli.rs` の `threshold_of`・席の側の `int_rule_of`）が 4 行を読んで同じ構築子 1 つで組む＝選定と退避の合図が同じ値を読む結び方は保つ。4 行のどれかが無い・不発効・形が違う周は既存どおり `RuleError` / `no-rule` で断る（既定値に倒さない）。強制の窓の字面が `WindowKind` の `parse` を通らない周も同じ断り。
  3. **席の逼迫度**: `reading` は数える窓（`counted` の規則は不変）ごとに自分の窓の閾値と比べ、以上の窓が 1 つでも在れば `Account` の variant `Over`。`Over` が運ぶ使用率と閾値は**引き金の窓**のもの（以上の窓のうち強制の窓を先・次に使用率の大きい順・同順位は `WINDOWS` の宣言順＝強制の窓が空の全窓同値の下では従来の「最大の使用率」と同じ値を運ぶ）で、比較と引き金の窓の選び方は行 g が入れ（全窓同値の下では従来と同じ結果）、強制の窓のときにその窓を `Over` に載せる形は行 h。退避の合図の字面（`account_pointer`）は不変（運ぶ実測値と閾値が引き金の窓の値になるだけ）。
  4. **判定行**（行 h）: 引き金が強制の窓の周だけ、判定行の `origin=account` の直後に `forced=<窓の字面>` の 1 語を足す（5 時間窓だけが以上の周・閾値未満の周の判定行は 1 byte も変わらない）。
  5. **強制の印**: tick は強制の退避の合図を送ると決めた周（FR29 と同じ除外と `signal_brake` を通った後・注入の直前＝write-ahead・その周は `seat/tick.rs` の合図の注入の手前に在るので印を書く呼出はそこに置き、印の口〔書く / 外す / 有無〕は行 i の write-set の `+` の file のうち席の側の新 module が持つ）に、席の置き場（`seat_dir`）へ印の file を 1 つ書く（1 行 JSON・`ts` / 口座 label / 窓 / 使用率 / 閾値 / tick が受けた作業記憶の置き場）。**判定に使うのは file の有無だけ**で、中身は出所の記録と断りの 1 行に載せる材料（中身が壊れていても印は効く）。注入が入力欄の門で断られた周も印は残る（席は次の tool 呼出の断りの 1 行から退避へ向かう）。
  ...
  7. **強制 cycle の guard**（PreToolUse・in-loop・新 module は行 i の write-set の `+` の file）: 門の位置は command guard の後・role guard の前（`polarity.rs` の `Guard` に variant `ForcedCycle` を `Command` の直後に足す＝門の順は宣言順）。(a) 最初に置き場の席 dir を列挙して印の有無を見る。どこにも印が無い周（席 dir の親がまだ無い置き場を含む）は pane も見ず tmux も撃たず記録も残さずに通す（毎 tool 呼出で tmux を撃たない・NFR5）。(b) 印が 1 つでも在る周だけ pane から自席の target を解き（`target_of_pane`）、自席の印が無ければ記録なしで通す。解けない周は下の 8。(c) 自席の印が在る周は **退避に要る操作だけ**を通す: Bash は、command を command guard と同じ区切りで segment に分け（区切る関数は `hook/command.rs` の 1 本を crate 内に公開して共有する＝区切りの規則を 2 面に持たない）、**全部の segment** の先頭語が器自身（role guard の `is_self` と同じ規則）で続く 2 語が閉じた列（`seat externalize`・`seat heartbeat`）に在り、command が置換（`$(`・backtick）と redirect の字を含まない周だけ。編集 tool（write-set guard と同じ集合 `GUARDED`）は、編集先が自席の席 dir の直下の入力 dir（1 段ちょうど・`seat_guard` の退避の口と同じく字句の段と link の実体の段で狭く取る）の中の周だけ。それ以外は deny（rc 2・stderr 1 行・記録 1 行 `forced-cycle-deny`）。読みの tool は PreToolUse の matcher に無いので元から通る。(d) 断りの 1 行は理由の語 `forced-cycle` と、印の中身から組んだ**次の手の 1 形**（`<NAME> seat externalize --target … --wm-dir … --anchor … --plan <入力 dir>/… --directives <入力 dir>/…`・role guard の `route` と同じ考え方＝席が source も skill の変数展開も使わずに退避へ進める・N2 の散文の手順を持たない）を運ぶ。判定は新 module の閉じた enum（通す理由を 1 つに畳まない・`seat_guard` の `SeatDecision` と同じ型）で持つ。
  8. **極性 = FailOpen**（`seat_guard` の cap guard と同じ側）: 自席に印が在るかを確かめられない周は 3 経路とも通し、記録に `forced-cycle-unresolved reason=<語>` を 1 行残す: 置き場の席 dir が在るのに列挙できない（`reason=place-unreadable`・印の有無そのものが分からない）／印が 1 つでも在るのに pane が無い・空（`reason=no-pane`）／印が 1 つでも在るのに pane から target を解けない（`reason=target-unresolved`・tmux が返らない・失敗する）。記録を残すのはこの 3 経路だけで、印がどこにも無いと確かめられた周は記録も tmux も無い（上の 7 (a)）。FailClosed にすると、host のどこかの席に印が在る間、target を解けない全部の session の Bash と編集が止まる＝器が自分の席を使えなくする（cap guard が測れない周を deny しないのと同じ理由）。pane を持たない session（席でない）も、印が在る周は `reason=no-pane` の 1 行を残して通す（印の在る間に解けないまま通った呼出を後から数えられる・FR21）。極性一覧には in-loop / fail-open で載る（C16.2・隠さない）。
...
- 触らない: `R-C9-1` の行の id と kind と形（Int・動くのは行 k の値と裁定 id だけ）・便用の選定と session 用の選定の鍵と順序（`pick` / `run_key`）・`counted` / `counts` の窓の数え方・`LIMIT_PCT`・退避の合図の字面と `signal_brake`・FR29 の除外・5 時間窓の席の挙動（従来どおりの合図で印も guard も無い）・context の軸（`seat.context_cap_pct` と cap guard）・妥協の通知の周（[seat-roles.md](./seat-roles.md) §18・退避の合図を送らない周なので印も書かない）・[account-autonomy.md](./account-autonomy.md)。同時に流す agent の数の上限は本 § の外（planner 側の投入量の題・閾値でも guard でも解けない）。
- 歯: **値と合図**（`seat_window_threshold_` 接頭辞）= 行 g（in-file・挙動不変の便なので型と純関数を窓別の値で直接測る）: (b) 型の 4 つの口（窓から閾値を引く・強制かを問う・全窓同値の構築子・4 行の値から組む構築子が未知の窓の語を断る）と、窓別の値を入れた `standing` / `reading` の比較（7 日窓 90% は 95 の下で候補・未満、5 時間窓 85% は以上）と引き金の窓の選び方、property（選ばれた口座は数える窓の全部が自分の窓の閾値未満）。行 k（e2e・挙動が変わる便）: base は特例の便の後の manifest（`R-C9-1` = 95 が全部の窓に効く）なので、挙動が変わるのは 5 時間窓の 85 以上 95 未満の帯: (a) 5 時間窓 90% の口座は `fleet select` の session 用の候補から外れ（base は候補に残る → RED）、7 日窓 90% は候補に残り 95% で外れる／(c) rules 行 3 つが `RuleKind` の `ALL` と `rules validate` の外形に載り、`R-C9-1` の値が 85・裁定 id が 03:28Z に戻り（既存の pin の歯と外形 snapshot の `rules get R-C9-1` の行を写す）、行が 1 つ欠けた manifest を断る＝`--rules` を読む側の `fleet select` は e2e で、埋め込み manifest を読む席の側（読み元は変えない）は渡された manifest から組む pure な読み手の in-file の歯で測る（行が欠けた・不発効・形が違う manifest をそれぞれ理由を名指して断る・tick の e2e では測らない）／(d) 席: 5 時間窓 90% の席の tick は閾値 85 の退避の合図を送り（base は閾値未満 → RED）、7 日窓 96% の席は閾値 95 の退避の合図・7 日窓 90% の席は打刻の合図の側へ進み、立て直しの選定は 5 時間窓 90% の口座と 7 日窓 95% の口座を外す＝選定と席が同じ便で同じ値に切り替わる。席の e2e は閾値を**埋め込み manifest** から読む（席の `--rules` の fixture は `R-C9-1` の行を持たない）ので、特例の便が 95 の両側へ直した既存の歯（`tests/e2e/seat.rs`・`tests/e2e/seat/account.rs`・`tests/e2e/seat/cycle.rs`・`tests/e2e/seat/tick.rs`＝85 と 95 を弁別する歯 3 本〔名は `seat_threshold_95_` で始まる・実測値 90 を 5 時間窓に置く〕は本便で結果が**逆**になるので、5 時間窓 85 の側の歯として名と期待を直す）を、fixture が実測を置く窓に合わせて窓別の値の両側へ直す（`acct_measured` は 5 時間窓に使用率・7 日窓に 1% を置く＝既存の歯は 5 時間窓の 85 の両側・字面は `閾値 85%`）。tmux の席を立てる歯を足す行（h / i / j / k）は nextest の tmux 群の名の列も同じ便で足す（`cargo xtask check` の `nextest-tmux-group` が drift を落とす）。行 h: (d') 7 日窓 96% の席の判定行に `forced=seven_day` が載り、5 時間窓 85% の席の判定行は従来と同じ字面（`forced=` が無い）。**guard**（`seat_forced_cycle_` 接頭辞）= (e) 強制の合図を送った周に印が書かれ、5 時間窓の合図の周には書かれない／(f) 印の在る席の pane からの Bash・Edit は deny で、断りの 1 行が次の手の形を運び、`seat externalize` / `seat heartbeat` の Bash と入力 dir への Write は通る（base は全部通る → RED）／(g) 別の席の印だけが在る周・印が無い周は通り、印が無い周は tmux を撃たない（偽 tmux の呼出記録が 0 行）／(h) 通す側の狭さ: `seat externalize` に別の segment を繋いだ command・置換を含む command・入力 dir の名の link で外を指す編集先は deny／(i) 確かめられない 3 経路を別々に測る: 印が在り pane が無い周（`reason=no-pane`）・印が在り tmux が失敗する周（`reason=target-unresolved`）・席 dir が在るのに列挙できない周（`reason=place-unreadable`）はそれぞれ通して記録が 1 行残り、印がどこにも無い周は pane が無くても記録が 0 行／(j) 印は externalize の成功・立て直し・強制でないと測れた tick の 3 つで外れ、外した名が残る（消えていない）／(k) 極性一覧に 1 行増え集計が動く／(l) 行 j: e2e は挙動だけ（agent を起こす tool は印の在る席で deny・印の無い席で通る）を測り、matcher と guard の対象集合の一致は guard の module の in-file の歯（tracked の hooks.json を埋め込みで読み、PreToolUse の matcher を割った集合と対象集合を比べる）が測る。
...
- 歯（名に `plugin_dir_` を含み、scribe2 側の 3 本は行 l の verify が名で撃つ〔既存の歯 `pipe_spawn_skips_plugin_dir_that_is_a_symlink` を filter に拾わない〕・xtask は `genmanifest_plugin_dir_`）: (a) xtask in-file `genmanifest_plugin_dir_places_three_files_under_plugin_and_points_source_there` = fixture の root で generate すると 3 生成物が plugin/ 配下（marketplace は root）に在り `source` が ./plugin（base は root 直下 → RED）。(b) `seat_launch_plugin_dir_is_anchor_plugin`（`tests/e2e/seat/launch.rs`）= 起動行の 1 つ目の `--plugin-dir` が anchor の下の plugin/。(c) `pipe_spawn_plugin_dir_copies_under_plugin`（`tests/e2e/pipe/spawn.rs`）= 便の worktree の plugin/ 配下に plugin.json と hooks.json が写される。(d) `doctor_consumers_plugin_dir_reads_hooks_under_plugin`（`tests/e2e/seat/account.rs`）= anchor の plugin/ 配下の hooks.json の digest が読み込み元の行に出る。tmux を立てる歯を足す周は `.config/nextest.toml` の tmux 群の名の列も同じ便で足す。移動の元の path を backtick で契約表の行の節が名指すのは [working-memory.md](./working-memory.md) §12（行 a / c）だけ（走査 2026-09-17・本節は backtick を持たない）＝同じ便で移動先の字面へ直す（名指しの検査が main で落ちない）。
...
[[contract]]
id = "h"
title = "強制の窓を判定行に出す（合図）— 引き金が強制の窓の周は Account の Over がその窓を運び、判定行の origin=account の直後に forced=<窓> の 1 語が出る（値の読み・合図の字面・除外・brake は不変）"
req = ["FR38", "FR29"]
section = "15"
depends = ["k"]
write-set = ["crates/scribe2/src/seat/tick.rs", "crates/scribe2/src/seat/tick/account.rs", "crates/scribe2/src/seat/tick/render.rs", "crates/scribe2/tests/e2e/seat/account.rs", ".config/nextest.toml"]
verify = ["cargo nextest run -p scribe2 --no-tests=fail seat_window_threshold_forced_"]
size = "S"
done = "7 日窓 96% の席の判定行に forced=seven_day が載り、5 時間窓だけが閾値以上の席と閾値未満の席の判定行は従来と同じ字面"
...
section = "15"
depends = ["i"]
write-set = ["crates/xtask/src/genmanifest.rs", "hooks/hooks.json", "crates/scribe2/src/hook/", "crates/scribe2/tests/e2e/hook.rs", ".config/nextest.toml"]
verify = ["cargo nextest run -p scribe2 --no-tests=fail seat_forced_cycle_agent_", "cargo nextest run -p scribe2 --lib --no-tests=fail seat_forced_cycle_matcher_"]
size = "S"
done = "印の在る席の agent を起こす tool の呼出が同じ断りで止まり、印の無い席では通り、生成 hooks.json の matcher にその名が載って gen-manifest の検査が緑"

[[contract]]
id = "k"
title = "session 用の閾値を窓別の値に切り替える（値）— rules 行 R-C9-1.seven-day / R-C9-1.seven-day-model（95）と R-C9-1.forced-windows を足し、選定側（fleet select）と席側（tick / launch / 立て直し）の読み手の全部を同じ便で 4 行の構築子へ切り替える（R-C9-1 は 5 時間窓の 85 に戻し裁定 id を user 2026-09-17T03:28Z に・足す 3 行は裁定 user 2026-09-17T03:50Z）"
req = ["FR36", "FR38"]
section = "15"
depends = ["g"]
write-set = ["rules/manifest.toml", "crates/scribe2/src/rules/mod.rs", "crates/scribe2/tests/e2e/rules.rs", "docs/design/rules-manifest.md", "crates/scribe2/tests/e2e/snapshots/e2e__rules__rules_external_form.snap", "crates/scribe2/src/fleet/cli.rs", "crates/scribe2/src/seat/mod.rs", "crates/scribe2/src/seat/cli.rs", "crates/scribe2/src/seat/tick.rs", "crates/scribe2/tests/e2e/fleet.rs", "crates/scribe2/tests/e2e/seat.rs", "crates/scribe2/tests/e2e/seat/account.rs", "crates/scribe2/tests/e2e/seat/cycle.rs", "crates/scribe2/tests/e2e/seat/tick.rs", ".config/nextest.toml"]
verify = ["cargo nextest run -p scribe2 --no-tests=fail seat_window_threshold_select_", "cargo nextest run -p scribe2 --no-tests=fail seat_window_threshold_tick_", "cargo nextest run -p scribe2 --lib --no-tests=fail seat_window_threshold_rows_"]
size = "M"
done = "fleet select の session 用と席の tick と立て直しの選定が同じ便で 7 日窓とモデル別窓は 95・5 時間窓は 85 に切り替わり、4 行のどれかが欠けた manifest は選定側の口も席側の読み手も理由を名指して断り、便用の選定と選定の順序は不変"
[[contract]]
id = "l"
