...
verify = ["cargo nextest run -p scribe2 --no-tests=fail seat_window_threshold_select_", "cargo nextest run -p scribe2 --no-tests=fail seat_window_threshold_tick_"]
size = "M"
done = "fleet select の session 用と席の tick と立て直しの選定が同じ便で 7 日窓とモデル別窓は 95・5 時間窓は 85 に切り替わり、4 行のどれかが欠けた manifest は選定も tick も断り、便用の選定と選定の順序は不変"
<!-- contracts:end -->
