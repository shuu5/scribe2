...
[[contract]]
id = "k"
title = "session 用の閾値を窓別の値に切り替える（値）— rules 行 R-C9-1.seven-day / R-C9-1.seven-day-model（95）と R-C9-1.forced-windows を足し、選定側（fleet select）と席側（tick / launch / 立て直し）の読み手の全部を同じ便で 4 行の構築子へ切り替える（R-C9-1 は 5 時間窓 85 のまま・裁定 user 2026-09-17T03:50Z）"
req = ["FR36", "FR38"]
section = "15"
depends = ["g"]
