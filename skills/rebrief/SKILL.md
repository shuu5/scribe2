...
- **`[WM]` の状態**: `found`（現在の sid 名義）/ `candidate`（自席だが別 sid＝`/clear` 後の**通常形**・
  ...
  別席の退避物が `[WM]` に出ることはない。
- **`[PLUGIN] … drift=<語>`** はこの席が起動した hook 集合（読み込み元の記録）と今の hooks.json / この器の本体の
  食い違い（判断材料であって規則ではない）。`drift=hooks`（`hooks+binary` も）は「この席は次の周に器の手で作り直される」
  ＝長い仕事を始めず退避に備える。`drift=binary` は「次の hook から新しい本体」（席は作り直されない）。`unrecorded` /
  `unreadable` は記録の不在 / 不読＝「食い違いなし」と読み替えない。
