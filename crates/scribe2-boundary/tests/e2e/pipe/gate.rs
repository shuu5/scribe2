...
fn pipe_regate_returns_gated_fail_to_implemented_on_the_same_worktree() {
    ...
    // §24 形 4 の登録 row の退役で 23 → 24（regate は種別を足さない）。
    assert_eq!((vessel::fleet::STAGES.len(), vessel::fleet::KINDS.len()), (11, 24), "段と event の種別は増えない");
    clean(&[&repo, &state]);
}
...
/// 先端になる。main の sha は動かず、記帳は 1 件で、段の種別 11 個と event の種別 24 個（account-lifecycle.md §19 形 3 の
/// 群の逼迫の通知で 19 → 20・§20 形 5 / 6 の群の移動の 3 種で 20 → 23・§24 形 4 の登録 row の退役で 23 → 24）は増えない。
#[test]
